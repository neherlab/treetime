...
def get_tree_events(tt):
    tree_events_tt = sorted(
        [(n.time_before_present, n.name, int(n.bad_branch)) for n in tt.tree.find_clades()], key=lambda x: -x[0]
    )
...
def compare(new_times_and_names, old_times_and_names):
    ...
    df = pd.DataFrame(
        dict(
            time=np.float_(new_times_and_names.iloc[:, 0]),
            difference=(np.float_(new_times_and_names.iloc[:, 0]) - np.float_(old_times_and_names.iloc[:, 0])),
            bad_branch=bad_branches,
        ),
        index=new_times_and_names.index,
    )
...
def compare_dist(node_num, node_fft, dist_node_num, dist_node_fft, dist_name, xlimits=None):
    fig = plt.figure()
    plt.plot(
        dist_node_num.x, dist_node_num.prob_relative(dist_node_num.x), marker='o', linestyle='', ms=2, label='numerical'
    )
...
if __name__ == '__main__':
    ...
    tt_kwargs = {'clock_rate': clock_rate, 'time_marginal': 'assign'}
    coal_kwargs = {'Tc': 10000, 'time_marginal': 'assign'}
    ...
    def closer_look_differences():
        ...
        for n in list(large_differences.index):
            ...
            if test_node.name != tt.tree.root.name and not test_node.marginal_pos_LH.is_delta:
                ...
                compare_dist(
                    test_node, test_node_fft, test_node.marginal_pos_LH, test_node_fft.marginal_pos_LH, 'pos_LH'
                )
                # compare msg_from_parent
                compare_dist(
                    test_node,
                    test_node_fft,
                    test_node.msg_from_parent,
                    test_node_fft.msg_from_parent,
                    'msg_from_parent',
                )
                ...
                compare_dist(
                    test_node,
                    test_node_fft,
                    test_node.subtree_distribution,
                    test_node_fft.subtree_distribution,
                    'subtree_dist',
                )
                ...
                compare_dist(
                    test_node,
                    test_node_fft,
                    test_node.branch_length_interpolator,
                    test_node_fft.branch_length_interpolator,
                    'branch length dist',
                    xlimits='empty',
                )
                ...
                compare_dist(
                    test_node,
                    test_node_fft,
                    test_node.marginal_pos_Lx,
                    test_node_fft.marginal_pos_Lx,
                    'marginal_pos_Lx',
                    xlimits=xlimits,
                )
                ...
                print(
                    'The parent of numerical is: '
                    + str(test_node.up.name)
                    + ' the parent of fft is: '
                    + str(test_node_fft.up.name)
                )
                compare_dist(
                    test_node.up,
                    test_node_fft.up,
                    test_node.up.marginal_pos_LH,
                    test_node_fft.up.marginal_pos_LH,
                    'pos_LH parent',
                )
                ...
                for c in test_node.clades:
                    ...
                    print(
                        'Time difference is ' + str(test_node.time_before_present - test_node_fft.time_before_present)
                    )
                    print(
                        'Time of numerical:'
                        + str(test_node.time_before_present)
                        + ' time of fft:'
                        + str(test_node_fft.time_before_present)
                    )
    ...
    def compare_multiply_functions(nodes_of_interest):
        ...
        for n in nodes_of_interest:
            ...
            msgs_to_multiply_tt_fft = [
                child.marginal_pos_Lx for child in test_node_fft.clades if child.marginal_pos_Lx is not None
            ]
            msgs_to_multiply_tt = [
                child.marginal_pos_Lx for child in test_node.clades if child.marginal_pos_Lx is not None
            ]
            ...
            compare_dist(
                test_node,
                test_node_fft,
                subtree_distribution_tt,
                subtree_distribution_tt_fft,
                'multiplied Lx of children',
            )
