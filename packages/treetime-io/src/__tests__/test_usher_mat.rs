#[cfg(test)]
mod tests {
  use crate::usher_mat::{usher_mat_pb_write_bytes, usher_mat_pb_write_file};
  use eyre::Report;
  use pretty_assertions::assert_eq;
  use std::fs;
  use tempfile::TempDir;
  use util_usher_mat::usher_mat_pb_read_bytes;

  #[test]
  fn test_usher_mat_pb_write_file_matches_encoded_bytes() -> Result<(), Report> {
    let tree = helpers::tree();
    let dir = TempDir::new()?;
    let path = dir.path().join("tree.pb");

    usher_mat_pb_write_file(&path, &tree)?;

    assert_eq!(usher_mat_pb_write_bytes(&tree)?, fs::read(&path)?);
    Ok(())
  }

  #[test]
  fn test_usher_mat_pb_write_file_roundtrip() -> Result<(), Report> {
    let tree = helpers::tree();
    let dir = TempDir::new()?;
    let path = dir.path().join("tree.pb");

    usher_mat_pb_write_file(&path, &tree)?;

    assert_eq!(tree, usher_mat_pb_read_bytes(&fs::read(&path)?[..])?);
    Ok(())
  }

  mod helpers {
    use crate::usher_mat::UsherTree;

    pub fn tree() -> UsherTree {
      UsherTree {
        newick: "(A:1,B:2)root;".to_owned(),
        ..UsherTree::default()
      }
    }
  }
}
