...
mod tests {
  use crate::nwk::{
    ...
    NwkWriteOptions, nwk_read_str, nwk_write_str_with,
  };
  use eyre::Report;
  use maplit::btreemap;
  ...
  fn find_node_comments(
    graph: &Graph<AnnotNode, AnnotEdge, ()>,
    ...
    graph
      ...
      .find_map(|node| {
        ...
        (names[&node.key()].as_deref() == Some(name)).then(|| node.payload().read_arc().comments.clone())
      })
  ...
  fn test_nwk_annotations_beast_single_attribute() -> Result<(), Report> {
    let NwkParse { graph, names, .. } = nwk_read_str::<AnnotNode, AnnotEdge, ()>("(A[&prob=0.95]:0.1,B:0.2);")?;
    let comments = find_node_comments(&graph, &names, "A");
  ...
  fn test_nwk_annotations_beast_multiple_attributes() -> Result<(), Report> {
    let NwkParse { graph, names, .. } =
      nwk_read_str::<AnnotNode, AnnotEdge, ()>(r#"(A[&prob=0.95,country="USA"]:0.1,B:0.2);"#)?;
    let comments = find_node_comments(&graph, &names, "A");
  ...
  fn test_nwk_annotations_nhx_attributes() -> Result<(), Report> {
    let NwkParse { graph, names, .. } = nwk_read_str::<AnnotNode, AnnotEdge, ()>("(A[&&NHX:S=human:B=90]:0.1,B:0.2);")?;
    let comments = find_node_comments(&graph, &names, "A");
  ...
  fn test_nwk_annotations_plain_comment_not_wired() -> Result<(), Report> {
    let NwkParse { graph, names, .. } = nwk_read_str::<AnnotNode, AnnotEdge, ()>("(A[a note]:0.1,B:0.2);")?;
    let comments = find_node_comments(&graph, &names, "A");
  ...
  fn test_nwk_annotations_unannotated_tree_unchanged() -> Result<(), Report> {
    let NwkParse { graph, names, .. } = nwk_read_str::<AnnotNode, AnnotEdge, ()>("(A:0.1,B:0.2)root;")?;
    let comments_a = find_node_comments(&graph, &names, "A");
    let comments_b = find_node_comments(&graph, &names, "B");
    let comments_root = find_node_comments(&graph, &names, "root");
  ...
  fn test_nwk_annotations_mixed_dialects_per_node() -> Result<(), Report> {
    let NwkParse { graph, names, .. } =
      nwk_read_str::<AnnotNode, AnnotEdge, ()>("(A[&k=v]:0.1,B[&&NHX:S=human]:0.2);")?;
    let comments_a = find_node_comments(&graph, &names, "A");
    let comments_b = find_node_comments(&graph, &names, "B");
  ...
  fn test_nwk_annotations_beast_boolean_value() -> Result<(), Report> {
    let NwkParse { graph, names, .. } = nwk_read_str::<AnnotNode, AnnotEdge, ()>("(A[&fixed=TRUE]:0.1,B:0.2);")?;
    let comments = find_node_comments(&graph, &names, "A");
  ...
  fn test_nwk_annotations_beast_array_value() -> Result<(), Report> {
    let NwkParse { graph, names, .. } = nwk_read_str::<AnnotNode, AnnotEdge, ()>("(A[&hpd={1.0,2.0}]:0.1,B:0.2);")?;
    let comments = find_node_comments(&graph, &names, "A");
  ...
  fn test_nwk_annotations_roundtrip_beast() -> Result<(), Report> {
    ...
    let NwkParse { graph, names, .. } = nwk_read_str::<AnnotNode, AnnotEdge, ()>(input)?;
    ...
    let output = nwk_write_str_with(&graph, &names, &edge_branch_lengths(&graph), &options, &providers)?;
  ...
  fn test_nwk_annotations_root_annotated() -> Result<(), Report> {
    let NwkParse { graph, names, .. } = nwk_read_str::<AnnotNode, AnnotEdge, ()>("(A:0.1,B:0.2)root[&date=2020.5];")?;
    let comments = find_node_comments(&graph, &names, "root");
