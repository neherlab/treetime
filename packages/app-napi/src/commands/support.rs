//! Shared output-planning and tree-ordering steps for the N-API command orchestrations.
//!
//! The N-API client always requests a command's default `--output-all` file set into one directory
//! and applies the default topology ordering before writing, matching the command-line client's
//! default behavior. These helpers build the fixed output plan and topology spec so each command
...
///
/// The N-API client requests the command's default set into one directory and, for the few requests that
