...
fn attach_seqs_to_graph(graph: &ReprGraph, partitions: &[PartitionParsimonyWithAln]) -> Result<(), Report> {
  for leaf in graph.get_leaves() {
    ...
    let partitions = partitions
      .iter()
      .map(
        |PartitionParsimonyWithAln { alphabet, aln, length }| -> Result<Box<dyn NodePartition>, Report> {
          ...
          Ok(Box::new(SparseNodePartition::new(&leaf_fasta.seq, alphabet)?))
        },
      )
      .try_collect()?;
  ...
  graph.par_iter_breadth_first_backward(|mut node| {
    ...
    for (_, edge) in &node.children {
      edge.write_arc().set_partitions(
        (0..n_partitions)
          .map(|_| Box::new(SparseEdgePartition::default()) as Box<dyn EdgePartition>)
          .collect_vec(),
      );
    }
