...
use crate::representation::graph_dense::{
  DenseEdge, DenseGraph, DenseNode, DenseSeqDis, DenseSeqEdge, DenseSeqInfo, DenseSeqNode,
};
...
fn marginal_dense_backward(graph: &DenseGraph, partitions: &[PartitionLikelihood]) {
...
fn run_marginal_dense_backward(
  ...
  node: &mut GraphNodeBackward<DenseNode, DenseEdge, ()>,
) -> Result<(), Report> {
...
fn marginal_dense_forward(graph: &DenseGraph, partitions: &[PartitionLikelihood]) {
...
fn run_marginal_dense_forward(
  partitions: &&[PartitionLikelihood],
  node: &mut GraphNodeForward<DenseNode, DenseEdge, ()>,
) {
  for si in 0..partitions.len() {
