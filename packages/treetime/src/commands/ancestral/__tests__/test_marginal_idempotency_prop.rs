  ...
  proptest! {
    ...
    fn test_prop_marginal_idempotency_dense(input in arb_marginal_input_small()) {
      let graph: GraphAncestral = nwk_read_str(&input.newick)
        .map_err(|e| TestCaseError::fail(format!("Failed to parse newick: {e}")))?;
      ...
      let log_lh_first = update_marginal(&graph, &partitions)
        .map_err(|e| TestCaseError::fail(format!("First update failed: {e}")))?;
      let log_lh_second = update_marginal(&graph, &partitions)
        .map_err(|e| TestCaseError::fail(format!("Second update failed: {e}")))?;
