...
pub(super) static NUC_ALPHABET: LazyLock<Alphabet> = LazyLock::new(Alphabet::default);

pub(super) const TREE_NEWICK: &str = "((A:0.1,B:0.2)AB:0.1,(C:0.2,D:0.12)CD:0.05)root:0.01;";

pub(super) fn gap_free_alignment() -> Result<Vec<FastaRecord>, Report> {
  read_many_fasta_str(
    indoc! {r#"
    >A
    ACGTACGTACGTACGT
...
pub(super) fn setup_sparse_only(
  graph: &GraphAncestral,
  aln: &[FastaRecord],
...
pub(super) fn get_branch_lengths(graph: &GraphAncestral) -> Vec<f64> {
  graph
    .get_edges()
    .iter()
