use crate::{alphabet, gtr::gtr::GTR, representation::{graph_sparse::{SparseGraph, SparseSeqEdge}, partitions_likelihood::PartitionLikelihood}};
use eyre::Report;
...
fn get_coefficients(edge: &SparseSeqEdge, gtr: &GTR) -> PartitionContribution {
  ...
  // TODO: add substitions
  let variable_positions: Vec<usize> = edge.msg_to_child.variable.keys().copied()
  ...
  let variable_states = variable_positions.iter().map(|pos| {
    ...
    let state_pair = if let Some(sub) = edge.subs.iter().find(|m| m.pos == *pos) {
      ...
      let parent = edge.msg_to_child.variable.get(&pos)
                        .or(edge.msg_to_parent.variable.get(&pos)).unwrap().state;
      let child = edge.msg_to_parent.variable.get(&pos)
                  .or(edge.msg_to_child.variable.get(&pos)).unwrap().state;
      (parent, child)
    };
    state_pair
  });
  ...
  for (&pos, (parent_state, child_state)) in zip(&variable_positions, variable_states) {
    let parent = if let Some(parent) = edge.msg_to_child.variable.get(&pos)
       { &parent.dis } else { edge.msg_to_child.fixed.get(&parent_state).unwrap() };

    let child = if let Some(child) = edge.msg_to_parent.variable.get(&pos)
       { &child.dis } else { edge.msg_to_parent.fixed.get(&child_state).unwrap() };
...
fn evaluate_sparse(
  coefficients: &Vec<PartitionContribution>,
  branch_length: f64,
) -> (f64, f64,f64, f64) {
  ...
  for coeffs in coefficients {
    ...
    for coeff in &coeffs.site_contributions {
      ...
      second_derivative += coeff.multiplicity * (&coeff.coefficients * &ev2_exp_ev).sum() / val - (coeff.multiplicity * (&coeff.coefficients * &ev_exp_ev).sum() / val).powi(2);
    }
  };
...
pub fn run_optimize_sparse(graph: &SparseGraph, partitions: &[PartitionLikelihood]) -> Result<(), Report> {
  ...
  graph.get_edges().iter_mut().for_each(|edge| {
    ...
    let coefficients = (0..n_partitions).map(|pi| {
      get_coefficients(&edge.sparse_partitions[pi], &partitions[pi].gtr)
    }).collect_vec();
    ...
    let (likelihood, log_likelihood, derivative, second_derivative) =
      evaluate_sparse(&coefficients, branch_length);
