...
pub fn compute_positional_likelihood(
  _partitions: &[Arc<RwLock<dyn PartitionTimetreeAll<NodeTimetree, EdgeTimetree>>>],
) -> Option<f64> {
...
pub fn compute_coalescent_likelihood(
  _partitions: &[Arc<RwLock<dyn PartitionTimetreeAll<NodeTimetree, EdgeTimetree>>>],
) -> Option<f64> {
...
pub fn compute_total_likelihood(
  partitions: &[Arc<RwLock<dyn PartitionTimetreeAll<NodeTimetree, EdgeTimetree>>>],
) -> Option<f64> {
  let seq_lh = compute_sequence_likelihood(partitions);
  let pos_lh = compute_positional_likelihood(partitions);
  let coal_lh = compute_coalescent_likelihood(partitions);
