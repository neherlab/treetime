...
mod tests {
  ...
  fn test_metadata_args_default_delimiters() {
    let args = TreetimeClockArgs::try_parse_from(["treetime", "--metadata=/dev/null"]).unwrap();
  ...
  fn test_metadata_args_explicit_delimiters() {
    let args =
      TreetimeClockArgs::try_parse_from(["treetime", "--metadata=/dev/null", "--metadata-delimiters", "|", ":"])
        .unwrap();
    assert_eq!(vec!['|', ':'], args.metadata_id.metadata_delimiters);
  }
}
