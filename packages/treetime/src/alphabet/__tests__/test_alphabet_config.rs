...
mod tests {
  ...
  fn test_alphabet_config_create_profile_map() {
    ...
    let profile_a = &profile_map[&AsciiChar(b'A')];
  ...
  fn test_alphabet_config_create_profile_map_treat_gap_as_unknown() {
    ...
    let profile_gap = &profile_map[&AsciiChar(b'-')];
    let profile_n = &profile_map[&AsciiChar(b'N')];
  ...
  fn test_alphabet_config_create_profile_map_no_gap_when_not_treat_as_unknown() {
    let config = make_valid_config();
    let profile_map = config.create_profile_map().unwrap();
    assert!(!profile_map.contains_key(&AsciiChar(b'-')));
  }
