...
mod tests {
  ...
  fn test_nij_orientation_dense() -> Result<(), Report> {
    ...
    let counts = partition.partition.count_transitions(&graph, &branch_lengths, &partition.node_states, &partition.backward, &partition.forward)?;
  ...
  fn test_ti_scaling_sparse(#[case] (bl1, bl2): (f64, f64)) -> Result<(), Report> {
    ...
    let counts1 = get_mutation_counts_fitch(&graph1, &fitch1, &branch_lengths1)?;
    let counts2 = get_mutation_counts_fitch(&graph2, &fitch2, &branch_lengths2)?;
  ...
  fn test_dense_sparse_consistency() -> Result<(), Report> {
    ...
    let dense = partition_d.partition.count_transitions(&graph_d, &branch_lengths_d, &partition_d.node_states, &partition_d.backward, &partition_d.forward)?;
