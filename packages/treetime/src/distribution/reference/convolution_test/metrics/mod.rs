//! Comprehensive convolution testing metrics
//!
//! This module provides a clean separation of different types of metrics used
//! to evaluate convolution algorithm performance:
//!
//! - **Aggregate**: Domain-wide scalar summaries and quality assessments
//! - **Pointwise**: Pure point-by-point error analysis (1:1 with evaluation grid)
//! - **Spatial**: Regional analysis and windowed spatial metrics
...
fn compute_grid_spacing(x: &Array1<f64>) -> eyre::Result<f64> {
  if x.len() < 2 {
    return Err(eyre::eyre!("Cannot compute grid spacing with fewer than 2 points"));
  }
  let dx = x[1] - x[0];
  if !dx.is_finite() || dx <= 0.0 {
    return Err(eyre::eyre!("Invalid grid spacing: {dx}"));
  }
  Ok(dx)
}
