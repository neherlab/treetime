...
mod tests {
  ...
  fn manual_indel_count_on_edge(sparse_partitions: &[PartitionMarginalSparse], edge_key: GraphEdgeKey) -> usize {
    sparse_partitions
      .iter()
      .map(|partition| partition.edges.get(&edge_key).map_or(0, |edge| edge.indels.len()))
      .sum()
  }
