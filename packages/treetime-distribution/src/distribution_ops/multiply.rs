...
/// N-ary generalization of [`with_composed_tails`]): soft `Linear` slopes add, a hard bound
/// dominates, `Error` dominates all. No tail is re-fit from the summed grid.
fn multiply_functions<Y: YAxisPolicy>(functions: &[&DistributionFunction<f64, Y>]) -> Result<Distribution<Y>, Report> {
  ...
  if !(left < right) {
    // Overlapping hard domains (checked above) always leave left < right, so this is a numerical
    // collapse. Guarding it keeps an empty product from silently poisoning every ancestor node.
...
/// N-ary generalization of the left column of [`multiplication_support_intersection`].
...
/// unbounded; only hard sides can separate domains. The N-ary generalization of
