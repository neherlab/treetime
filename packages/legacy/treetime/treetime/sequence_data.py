...
class SequenceData(object):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.seq_multiplicity = (
            seq_multiplicity or {}
        )  # possibly a dict mapping sequences to their read cound/sample count
        self.additional_constant_sites = (
            kwargs['additional_constant_sites'] if 'additional_constant_sites' in kwargs else 0
        )
        ...
        if isinstance(in_aln, MultipleSeqAlignment):
            ...
            for s in in_aln:
                if s.id == s.name:
                    ...
                    '<unknown' in s.name
                ):  # use s.id if s.name is BioPython default (change relative to previous, but what we want)
                    tmp_name = s.id
                else:
        ...
        if self.full_length:
            if self.is_sparse:
                if self.full_length != len(self.ref):
                    self.logger(
                        "SequenceData.aln: specified sequence length doesn't match reference length, ignoring sequence length.",
                        1,
                        warn=True,
                    )
                ...
                if self.full_length < in_aln.get_alignment_length():
                    raise AttributeError(
                        'SequenceData.aln: specified sequence length is smaller than alignment length!'
                    )
                elif self.full_length > in_aln.get_alignment_length():
                    self.logger(
                        "SequenceData.aln: specified sequence length doesn't match alignment length. Treating difference as constant sites.",
                        2,
                        warn=True,
                    )
    ...
    def ref(self, in_ref):
        ...
        if in_ref and isfile(in_ref):
            ...
            if not read_from_file:
                raise TypeError(
                    'SequenceData.ref: reference sequence file %s could not be parsed, fasta and genbank formats are supported.'
                )
        ...
        if self.sequence_type:
            if self.likely_alphabet != self.sequence_type:
                if self.sequence_type == 'nuc':
                    ...
                    self.logger(
                        'POSSIBLE ERROR: This looks like a nucleotide alignment, you indicated amino acids!',
                        0,
                        warn=True,
                    )
    ...
    def make_compressed_alignment(self):
        ...
        for pi in variable_positions:
            if self.is_sparse:
                pattern = np.array(
                    [self.aln[k][pi] if pi in self.aln[k] else self.ref[pi] for k in self.sequence_names]
                )
            else:
        ...
        if self.additional_constant_sites:
            character_counts = {
                c: np.sum(aln_transpose == c)
                for c in alphabets[self.likely_alphabet + '_nogap']
                if c not in [self.ambiguous, '-']
            }
            total = np.sum(list(character_counts.values()))
            additional_columns_per_character = [
                (c, int(np.round(self.additional_constant_sites * n / total))) for c, n in character_counts.items()
            ]
        ...
        # pylint: disable=unsubscriptable-object
        self.compressed_alignment = {k: tmp_compressed_alignment[i] for i, k in enumerate(self.sequence_names)}
    ...
    def full_to_sparse_sequence(self, sequence):
        ...
        if self.ref is None:
            raise TypeError(
                'SequenceData: sparse sequences can only be constructed when a reference sequence is defined'
            )
    ...
    def differences(self, seq1, seq2, seq1_compressed=True, seq2_compressed=True, mask=None):
        ...
        if self.is_sparse:
            if seq1_compressed:
                seq1 = self.compressed_to_sparse_sequence(seq1)
            if seq2_compressed:
                seq2 = self.compressed_to_sparse_sequence(seq2)
            ...
            if seq1_compressed:
                seq1 = self.compressed_to_full_sequence(seq1)
            if seq2_compressed:
                seq2 = self.compressed_to_full_sequence(seq2)
            if mask is None:
