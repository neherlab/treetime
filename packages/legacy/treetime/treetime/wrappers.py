...
def create_gtr(params):
    ...
    if custom_gtr:
        if model not in ['custom', 'infer']:
            print(
                f'Warning: you specified a GTR model `{model}` and a custom gtr path `{custom_gtr}`. TreeTime will load the custom model and ignore the parameter `--gtr {model}`.'
            )
...
def scan_homoplasies(params):
    ...
    ###########################################################################
    treeanc = TreeAnc(params.tree, aln=aln, ref=ref, gtr=gtr, verbose=1, fill_overhangs=True, rng_seed=params.rng_seed)
    ...
    corrected_branch_length = np.sum(
        [np.exp(-x.branch_length) * np.sinh(x.branch_length) for x in treeanc.tree.find_clades()]
    )
    corrected_terminal_branch_length = np.sum(
        [np.exp(-x.branch_length) * np.sinh(x.branch_length) for x in treeanc.tree.get_terminals()]
    )
    ...
    if params.detailed:
        ...
        print(
            ...
            + ''.join(['\n\t - %d occur %d times' % (n, mi) for mi, n in enumerate(multiplicities_terminal) if n])
        )
    ...
    print(
        ...
        + ''.join(
            [
                ...
                for mi, n in enumerate(multiplicities_positions)
                if n
            ]
        )
    )
            ...
            if len(val) > 1:
                print(
                    ...
                    % (
                        ...
                        mut[2],
                        len(val),
                        ' '.join([drms[mut[1]]['gene'], drms[mut[1]]['drug'], drms[mut[1]]['alt_base'][mut[2]]])
                        if mut[1] in drms
                        else '',
                    )
                )
    ...
    if params.detailed:
        if params.drms:
            ...
            terminal_mutations_sorted = sorted(
                terminal_mutations.items(), key=lambda x: len(x[1]) - 0.1 * x[0][1] / L, reverse=True
            )
    ...
    if params.detailed:
        if params.drms:
            ...
            for name, val in mutation_by_strain_sorted[: params.n]:
                if len(val):
                    print('\t%s\t%d\t%d' % (name, len(val), len([mut for mut, l in val if mut[1] in drms])))
        else:
            print(
...
def arg_time_trees(params):
    ...
    for i, (tree, mask) in enumerate(zip(arg_params['trees'], arg_params['masks'])):
        ...
        tt = setup_arg(
            tree,
            arg_params['alignment'],
            arg_params['combined_mask'],
            mask,
            dates,
            arg_params['MCCs'],
            gtr=gtr,
...
def run_timetree(myTree, params, outdir, tree_suffix='', prune_short=True, method_anc='probabilistic'):
    ...
    try:
        ...
        if params.coalescent in ['opt', 'const', 'skyline']:
            ...
            raise TreeTimeError(
                'unknown coalescent model specification, has to be either '
    ...
    if type(coalescent) == float and coalescent > 0 and coalescent < myTree.one_mutation:
        raise TreeTimeError(
            f'coalescent time scale is too low, should be at least distance'
    ...
    try:
        success = myTree.run(
            ...
            clock_filter_method=params.clock_filter_method,
    ...
    if coalescent in ['skyline', 'opt', 'const']:
        ...
        if coalescent == 'skyline':
            ...
            print(
                " --N_e: \t %1.2e \tcorresponding 'effective population size' assuming %1.2e gen/year\n"
    ...
    label_func = lambda x: (
        x.name if x.is_terminal() and ((leaf_count < 30 and (not params.no_tip_labels)) or params.tip_labels) else ''
    )

    plot_vs_years(myTree, show_confidence=False, label_func=label_func, confidence=0.9 if calc_confidence else None)
...
def reconstruct_discrete_traits(
    tree,
    traits,
    missing_data='?',
    pc=1.0,
    ...
    if weight_dict is not None:
        ...
        weights = np.array(
            [weight_dict[letter_to_state[c]] if letter_to_state[c] in weight_dict else mean_weight for c in alphabet],
            dtype=float,
        )
        weights /= weights.sum()
    ...
    ###########################################################################
    treeanc = TreeAnc(
        tree, gtr=mugration_GTR, verbose=verbose, ref='A', convert_upper=False, one_mutation=0.001, rng_seed=rng_seed
    )
    treeanc.use_mutation_length = False
    pseudo_seqs = {
        n.name: {0: reverse_alphabet[traits[n.name]] if n.name in traits else missing_char}
        for n in treeanc.tree.get_terminals()
    }
    ...
    treeanc.infer_ancestral_sequences(
        infer_gtr=False, store_compressed=False, marginal=True, normalized_rate=False, reconstruct_tip_states=True
    )
...
def mugration(params):
    ...
    if os.path.isfile(params.states):
        states = pd.read_csv(params.states, sep='\t' if params.states[-3:] == 'tsv' else ',', skipinitialspace=True)
    else:
    ...
    if params.name_column:
        if params.name_column in states.columns:
            ...
            print(
                "Error: specified column '%s' for taxon name not found in meta data file with columns: "
                % params.name_column
                + ' '.join(states.columns)
            )
            return 1
    ...
    leaf_to_attr = {
        x[taxon_name]: str(x[attr]) for xi, x in states.iterrows() if x[attr] != params.missing_data and x[attr]
    }
...
def estimate_clock_model(params):
    ...
    if params.clock_filter:
        ...
        myTree.clock_filter(
            n_iqd=params.clock_filter, reroot=params.reroot or 'least-squares', method=params.clock_filter_method
        )
