...
def _convolution_integrand(t_val, f, g, inverse_time=None, return_log=False):
    ...
    if tau_max <= tau_min:
        ...
        FG = Distribution(
            tau, fg, is_log=True, min_width=np.max([f.min_width, g.min_width]), kind='linear', assume_sorted=True
        )
        return FG
...
def _evaluate_convolution(t_val, f, g, n_integral=100, inverse_time=None, return_log=False):
    ...
    if (return_log and FG == ttconf.BIG_NUMBER) or (not return_log and FG == 0.0):  # distributions do not overlap
...
class NodeInterpolator(Distribution):
    ...
    def convolve_fft(cls, node_interp, branch_interp, fft_grid_size=FFT_FWHM_GRID_SIZE, inverse_time=True):
        ...
        if ratio < 1.0 / fft_grid_size and 4.0 * dt > node_interp.fwhm:
            ...
            log_scale_node_interp = node_interp.integrate(
                return_log=True, a=node_interp.xmin, b=node_interp.xmax, n=max(100, len(node_interp.x))
            )  # probability of node distribution
            if inverse_time:
                x = branch_interp.x + max(n_effsupport[0], node_interp._peak_pos - 4.0 * node_interp.fwhm)
                dist = Distribution(
                    x,
                    branch_interp(x - node_interp._peak_pos) - log_scale_node_interp,
                    min_width=max(node_interp.min_width, branch_interp.min_width),
                    is_log=True,
                )
            else:
                ...
                dist = Distribution(
                    x,
                    branch_interp(branch_interp.x) - log_scale_node_interp,
                    min_width=max(node_interp.min_width, branch_interp.min_width),
                    is_log=True,
                )
            return dist
            ...
            raise ValueError(
                'ERROR: Unexpected behavior: branch distribution is much narrower than the node distribution.'
            )
        else:
            ...
            if margin < 1 or len(res) == 0:
                raise TreeTimeUnknownError(
                    'Error: Unexpected behavior detected in FFT function. '
            ...
            return cls(
                np.concatenate((Tleft, Tres_cropped, Tright)),
    ...
    def convolve(
        ...
        if max_or_integral not in ['max', 'integral']:
            raise Exception(
                "Max_or_integral expected to be 'max' or 'integral', got " + str(max_or_integral) + ' instead.'
            )

        def conv_in_point(time_point):
            if max_or_integral == 'integral':  # compute integral of the convolution
                return _evaluate_convolution(
                    time_point,
                    node_interp,
                    branch_interp,
                ...
                return _max_of_integrand(
                    time_point, node_interp, branch_interp, return_log=True, inverse_time=inverse_time
                )
        ...
        t_grid_0 = np.unique(
            np.concatenate([grid_zero_left, grid_left[:-1], grid_center, grid_right[1:], grid_zero_right])
        )
        ...
        refine_factor = np.minimum(
            np.minimum(
                np.array(np.floor(np.sqrt(interp_error / (rel_tol * (1 + (dy / yc) ** 4)))), dtype=int),
        ...
        if np.sum(insert_point_idx):
            add_x = np.concatenate(
                [
                    np.linspace(t1, t2, n + 2)[1:-1]
                    for t1, t2, n in zip(t_grid_0[1:-2], t_grid_0[2:-1], insert_point_idx)
                    if n > 0
                ]
            )
        ...
        res_t = Distribution(t_grid_0, t_0, is_log=True, min_width=node_interp.min_width, kind='linear')
