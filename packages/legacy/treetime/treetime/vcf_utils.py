...
def read_vcf(vcf_file, ref_file=None):
        ...
        for line in f:
            if line.startswith('##'):
                if current_block != 'meta-information':
                    raise TreeTimeError(
                        f'Malformed VCF file {vcf_file!r} - all the meta-information (lines starting with ##) must appear at the top of the file.'
                    )
                ...
                if not line.startswith(header_start):
                    raise TreeTimeError(
                        f'Malformed VCF file {vcf_file!r} - the header line must start with {header_start}'
                    )
                if current_block != 'meta-information':
                    raise TreeTimeError(
                        f'Malformed VCF file {vcf_file!r} - the header must immediately follow the meta-information lines'
                    )
                ...
                if metadata['chrom'] is None:
                    ...
                    raise TreeTimeError(
                        f'The VCF file {vcf_file!r} contains multiple chromosomes. TreeTime can not yet handle this.'
                    )
                ...
                if len(calls) != nsamp:
                    raise TreeTimeError(
                        f'Malformed VCF file {vcf_file!r} - the data lines have different number of calls than the number of samples defined in the header'
                    )
                ...
                for sname, sa in zip(samps, calls):
                    ...
                    if metadata['ploidy'] is None:
                        ...
                        raise TreeTimeError(
                            f'The VCF file {vcf_file!r} had genotype calls of ploidy {metadata["ploidy"]} but sample {sname!r} has a genotype of ploidy {ploidy}'
                        )
    ...
    if len(positions) == 0 and num_insertions == 0:
        raise TreeTimeError(
            f'VCF file {vcf_file!r} has no data-lines which we could extract genotype information from!'
        )
...
def write_vcf(tree_dict, file_name, mask=None):  # , compress=False):
    ...
    while i < len(positions):
        ...
        if any(pattern == '-'):
            if pos != 1:
                ...
                print(
                    fill(
                        'WARNING: You have a deletion in the first position of your'
    ...
    if 'inferred_const_sites' in tree_dict and explainedErrors != 0:
        print(
            fill(
                'Sites that were constant except for ambiguous bases were made'
