...
if __name__ == '__main__':
    ...
    def find_optimal_branch_fft_gridsizes(precision_fft, branch_grid_size):
        ...
        pp = PdfPages(
            './results/DifferencesComparison.pdf'
        )  # output the difference plots to visually assess if there is an issue
        ...
        for b in branch_grid_size:
            ...
            for prec in precision_fft:
                ...
                fig = plot_differences(
                    output_comparison[1], title='Differences branch grid ' + str(b) + ' fft grid ' + str(prec)
                )
                pp.savefig(fig)
            ...
            for i in range(1, len(divergence_times)):
                divergence_time_diff.append(
                    np.mean(
                        np.abs(divergence_times[i] - divergence_times[i - 1])
                        / (precision_fft[i] - precision_fft[i - 1])
                    )
                )
        ...
        for i in range(0, len(divergence_time_diff_list)):
            epsilon = 1e-9
            plt.plot(
                precision_fft[1 : len(divergence_times_fft_list[i][0])],
                divergence_time_diff_list[i][0],
                'o-',
                label=branch_grid_size[i],
            )
    ...
    time_fft, divergence_times_fft_list, divergence_time_diff_list = find_optimal_branch_fft_gridsizes(
        precision_fft, branch_grid_size
    )
    ...
    def find_optimal_branch_gridsizes(prec_fft, branch_grid_size):
        ...
        for b in branch_grid_size:
            ...
            output_comparison = compare(tree_events_numerical, tree_events_fft)
            plot_differences(
                output_comparison[1], title='Differences branch grid ' + str(b) + ' fft grid ' + str(prec_fft)
            )
        ...
        for i in range(1, len(divergence_times)):
            divergence_time_diff.append(
                np.nanmean(
                    np.abs(divergence_times[i] - divergence_times[i - 1])
                    / (branch_grid_size[i] - branch_grid_size[i - 1])
                )
            )

        memory_branch = np.array(memory_branch) / (10**6)
        accuracy_rate = np.array(divergence_time_diff) / 1e-8
        plot_accuracy_memory_time(
            branch_grid_size,
            accuracy_rate,
            time_branch,
            memory_branch,
            'Average Rate of Change Inferred Time [1e-8]',
            'average_rate_of_change',
        )
        plot_accuracy_memory_time(
            branch_grid_size, np.array(fft_LH), time_branch, memory_branch, 'tree log(LH) ', 'log(LH)'
        )
