...
gtr_params_description = (
    'GTR parameters for the model specified by '
...
reroot_description = (
    'Reroot the tree using root-to-tip regression. Valid choices are '
...
dates_description = (
    "csv file with dates for nodes with 'node_name, date' where date is float (as in 2012.15) or in ISO-format (YYYY-MM-DD). "
...
def add_aln_group(parser, required=True):
    ...
    parser.add_argument(
        '--vcf-reference', type=str, help='only for vcf input: fasta file of the sequence the VCF was mapped to.'
    )
...
def add_reroot_group(parser):
    ...
    parser.add_argument(
        ...
        type=float,
        default=3,
        help='excess variance associated with terminal nodes accounting for  overdispersion of the molecular clock',
    )
    parser.add_argument(
        '--covariation',
        action='store_true',
        help='Account for covariation when estimating rates or rerooting using root-to-tip regression, default False.',
    )
...
def add_gtr_arguments(parser):
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help='filename of pre-defined custom GTR model in standard TreeTime format',
    )
...
def add_anc_arguments(parser):
    ...
    parser.add_argument(
        ...
        default=False,
        action='store_true',
        help='overwrite ambiguous states on tips with the most likely inferred state',
    )
    parser.add_argument(
        '--report-ambiguous', default=False, action='store_true', help='include transitions involving ambiguous states'
    )
    parser.add_argument(
        '--method-anc',
        default='probabilistic',
        type=str,
        choices=['parsimony', 'fitch', 'probabilistic', 'ml'],
...
def add_timetree_args(parser):
    parser.add_argument(
        '--clock-rate', type=float, help="if specified, the rate of the molecular clock won't be optimized."
    )
    ...
    parser.add_argument(
        ...
        default='auto',
        type=str,
        choices=['auto', 'input', 'joint', 'marginal'],
    ...
    parser.add_argument(
        '--confidence',
        action='store_true',
        help='estimate confidence intervals of divergence times using the marginal'
    ...
    parser.add_argument(
        '--time-marginal',
        default='false',
        choices=['false', 'true', 'assign', 'always', 'only-final', 'never'],
    ...
    parser.add_argument('--coalescent', default='0.0', type=str, help=coalescent_description)
    parser.add_argument('--n-skyline', default='20', type=int, help='number of grid points in skyline coalescent model')
    parser.add_argument(
        '--gen-per-year',
        default='50.0',
        type=float,
    ...
    parser.add_argument(
        ...
        help='filename to save the plot to. Suffix will determine format (choices pdf, png, svg, default=pdf)',
    )
    parser.add_argument(
        '--plot-rtt',
    ...
    parser.add_argument(
        '--tip-labels', action='store_true', help='add tip labels (default for small trees with <30 leaves)'
    )
    parser.add_argument(
...
def make_parser():
    ...
    h_parser.add_argument(
        '--drms',
        type=str,
        help='TSV file containing DRM info. columns headers: GENOMIC_POSITION, ALT_BASE, DRUG, GENE, SUBSTITUTION',
    )
    ...
    a_parser.add_argument(
        '--marginal', default=False, action='store_true', help='marginal reconstruction of ancestral sequences'
    )
    ...
    m_parser.add_argument(
        ...
        required=True,
        type=str,
        help='csv or tsv file with discrete characters.\n#name,country,continent\ntaxon1,micronesia,oceania\n...',
    )
    m_parser.add_argument(
        '--weights',
        type=str,
        help='csv or tsv file with probabilities of that a randomly sampled '
    ...
    m_parser.add_argument(
        '--pc', type=float, default=1.0, help="pseudo-counts higher numbers will results in 'flatter' models"
    )
    ...
    c_parser.add_argument(
        '--prune-outliers', action='store_true', default=False, help='remove detected outliers from the output tree'
    )
    c_parser.add_argument(
        '--allow-negative-rate',
        required=False,
