...
class ClockTree(TreeAnc):
    ...
    def _assign_dates(self):
        ...
        for node in self.tree.find_clades(order='postorder'):
            if node.name in self.date_dict:
                ...
                if np.isscalar(tmp_date) and np.isnan(tmp_date):
                    self.logger(
                        'WARNING: ClockTree.init: node %s has a bad date: %s' % (node.name, str(tmp_date)), 2, warn=True
                    )
                    ...
                    try:
                        ...
                        self.logger(
                            'WARNING: ClockTree.init: node %s has a bad date: %s' % (node.name, str(tmp_date)),
                            2,
                            warn=True,
                        )
                        node.raw_date_constraint = None
        ...
        self.logger(
            'ClockTree._assign_dates: assigned date constraints to {} out of {} tips.'.format(
                self.tree.count_terminals() - bad_branch_counter, self.tree.count_terminals()
            ),
            1,
        )
    ...
    def _set_precision(self, precision):
        ...
        if precision in [0, 1, 2, 3]:
            ...
            if self.one_mutation and self.one_mutation < 1e-4 and precision < 2:
                self.logger(
                    'ClockTree._set_precision: FOR LONG SEQUENCES (>1e4) precision>=2 IS RECOMMENDED.'
    ...
    def _set_precision_fft(self, precision_fft, precision_branch='auto'):
        ...
        if type(precision_fft) is int:
            self.logger(
                'ClockTree.init._set_precision_fft: setting fft grid size explicitly,'
                ' fft_grid_points=%.3e' % (precision_fft),
                2,
            )
            self.fft_grid_size = precision_fft
        elif precision_fft != 'auto':
            raise UnknownMethodError(
                f"ClockTree: precision_fft needs to be either 'auto' or an integer, got '{precision_fft}'."
            )
        else:
            self.fft_grid_size = ttconf.FFT_FWHM_GRID_SIZE
        if type(precision_branch) is int:
            self.logger(
                'ClockTree.init._set_precision_fft: setting branch grid size explicitly,'
                ' branch_grid_points=%.3e' % (precision_branch),
                2,
            )
    ...
    def date2dist(self, val):
        if val is None:
            ...
            self.logger(
                'ClockTree.date2dist: Setting new molecular clock.'
                ' rate=%.3e, R^2=%.4f' % (val.clock_rate, val.r_val**2),
                2,
            )
            self._date2dist = val
    ...
    def setup_TreeRegression(self, covariation=True):
        ...
        Treg = TreeRegression(
            self.tree, tip_value=tip_value, branch_value=branch_value, branch_variance=branch_variance
        )
    ...
    def init_date_constraints(self, clock_rate=None, **kwarks):
        ...
        for node in self.tree.find_clades(order='postorder'):
            if node.up is None:
                ...
                node.branch_length_interpolator = BranchLenInterpolator(
                    node,
                    self.gtr,
            ...
            if hasattr(node, 'raw_date_constraint') and node.raw_date_constraint is not None:
                ...
                if hasattr(node, 'bad_branch') and node.bad_branch is True:
                    self.logger(
                        ...
                        ' Date constraint will be ignored!',
                        4,
                        warn=True,
                    )
    ...
    def _ml_t_joint(self):
        ...
        for node in self.tree.find_clades(order='postorder'):  # children first, msg to parents
            ...
            if node.bad_branch:
                ...
                if (
                    node.date_constraint is not None and node.date_constraint.is_delta
                ):  # there is a strict time constraint
                    ...
                    if hasattr(self, 'merger_model') and self.merger_model:
                        node.joint_pos_Lx = Distribution(
                            x,
                            -self.merger_model.integral_merger_rate(node.date_constraint.peak_pos)
                        ...
                        node.joint_pos_Lx = Distribution(
                            x, node.branch_length_interpolator(bl), min_width=self.min_width, is_log=True
                        )
                    ...
                    if hasattr(self, 'merger_model') and self.merger_model:
                        ...
                        if node.is_terminal():
                            msgs_to_multiply.append(
                                Distribution(
                                    time_points, -self.merger_model.integral_merger_rate(time_points), is_log=True
                                )
                            )
                        else:
                    ...
                    if node.up is None:  # this is the root, set dates
                        if hasattr(self, 'merger_model') and self.merger_model:
                            ...
                            subtree_distribution = Distribution.multiply(
                                [
                                    subtree_distribution,
                                    Distribution(
                                        subtree_distribution.x,
                                        self.merger_model.integral_merger_rate(subtree_distribution.x),
                                        is_log=True,
                                    ),
                                ]
                            )
        ...
        for node in self.tree.find_clades(order='preorder'):  # root first, msgs to children
            ...
            if (
                ...
                node.branch_length = node.joint_pos_Cx(max(node.joint_pos_Cx.xmin, node.up.time_before_present))

            # clean up tiny negative branch length, warn against bigger ones.
            if node.branch_length < 0:
                if node.branch_length > -2 * ttconf.TINY_NUMBER:
                    self.logger(
                        f'ClockTree - Joint reconstruction: correcting rounding error of {node.name} bl={node.branch_length:1.2e}',
                        4,
                    )
                else:
                    self.logger(
                        f'ClockTree - Joint reconstruction: NEGATIVE BRANCH LENGTH {node.name} bl={node.branch_length:1.2e}',
                        2,
                        warn=True,
                    )
    ...
    def timetree_likelihood(self, time_marginal):
        ...
        """
        if time_marginal:
            LH = self.tree.root.marginal_pos_LH.integrate(
                return_log=True, a=self.tree.root.marginal_pos_LH.xmin, b=self.tree.root.marginal_pos_LH.xmax, n=1000
            )
        else:
            LH = 0
    ...
    def _ml_t_marginal(self):
        ...
        self.logger(
            f'ClockTree - Marginal reconstruction using {method} convolution:  Propagating leaves -> root...', 2
        )
        # go through the nodes from leaves towards the root:
        for node in self.tree.find_clades(order='postorder'):  # children first, msg to parents
            if node.bad_branch:
                ...
                if (
                    ...
                    msgs_to_multiply.extend(
                        [child.marginal_pos_Lx for child in node.clades if child.marginal_pos_Lx is not None]
                    )
                    ...
                    if node.up is None:  # this is the root, set dates
                        ...
                        if hasattr(self, 'merger_model') and self.merger_model:
                            # Removed merger rate must be added back at the root as nolonger an internal node
                            node.marginal_pos_LH = Distribution.multiply(
                                [
                                    node.subtree_distribution,
                                    Distribution(
                                        node.subtree_distribution.x,
                                        self.merger_model.integral_merger_rate(node.subtree_distribution.x),
                                        is_log=True,
                                    ),
                                ]
                            )
                        ...
                        if self.use_fft:
                            res, res_t = (
                                NodeInterpolator.convolve_fft(
                                    node.subtree_distribution, node.branch_length_interpolator, self.fft_grid_size
                                ),
                                None,
                            )
                        else:
        ...
        for node in self.tree.find_clades(order='preorder'):
            ...
            if (node.date_constraint is not None) and (not node.bad_branch) and node.date_constraint.is_delta:
                ...
                node.msg_from_parent = (
                    None  # if internal node has a delta constraint no previous information is passed on
                )
            ...
            # all other cases (All internal nodes + unconstrained terminals)
                ...
                if node.marginal_pos_Lx is not None:
                    if len(parent.clades) < 5:
                        ...
                        complementary_msgs.extend(
                            [
                                sister.marginal_pos_Lx
                        ...
                        complementary_msgs = [
                            Distribution.divide(parent.product_of_child_messages, node.marginal_pos_Lx)
                        ]
                            ...
                            time_points = np.unique(
                                np.concatenate(
                                    [
                                        time_points,
                                        np.linspace(
                                            np.min([x.xmin for x in complementary_msgs]),
                                            np.max([x.xmax for x in complementary_msgs]),
                                            50,
                                        ),
                                        np.linspace(
                                            np.min([x.effective_support[0] for x in complementary_msgs]),
                                            np.max([x.effective_support[1] for x in complementary_msgs]),
                                            50,
                                        ),
                                    ]
                                )
                ...
                self.logger(
                    ...
                    ' with %d points at node %s' % (len(res.x), node.name),
                    4,
                )
                ...
                if self.debug:
                    ...
                    if nsign_changed > 1:
                        ...
                        plt.plot(
                            ...
                            node.branch_length_interpolator(node.branch_length_interpolator.x)
                            - node.branch_length_interpolator.peak_val,
                            '-o',
                        )
    ...
    def calc_rate_susceptibility(self, rate_std=None, params=None):
        ...
        if self.clock_model['slope'] < 0:
            raise ValueError(
                'ClockTree.calc_rate_susceptibility: rate estimate is negative. In this case the heuristic treetime uses to account for uncertainty in the rate estimate does not work. Please specify the clock-rate and its standard deviation explicitly via CLI parameters or arguments.'
            )
        ...
        self.make_time_tree(**params)
        self.logger(
            '###ClockTree.calc_rate_susceptibility: rate: %f, LH:%f' % (current_rate, self.tree.positional_LH), 2
        )
        for n in self.tree.find_clades():
    ...
    def date_uncertainty_due_to_rate(self, node, interval=(0.05, 0.095)):
        ...
        if hasattr(node, 'numdate_rate_variation'):
            ...
            nsig = [np.sqrt(2.0) * erfinv(-1.0 + 2.0 * x) if x * (1.0 - x) else 0 for x in interval]
    ...
    def combine_confidence(self, center, limits, c1=None, c2=None):
        ...
        return np.array([max(limits[0], min_val), min(limits[1], max_val)])
    ...
    def get_confidence_interval(self, node, interval=(0.05, 0.95)):
        ...
        if hasattr(node, 'marginal_inverse_cdf'):
            min_date, max_date = [
                self.date2dist.to_numdate(x) for x in (node.marginal_pos_LH.xmax, node.marginal_pos_LH.xmin)
            ]
        ...
        return self.combine_confidence(
            node.numdate, (min_date, max_date), c1=rate_contribution, c2=mutation_contribution
        )
    ...
    def get_max_posterior_region(self, node, fraction=0.9):
        ...
        if node.marginal_pos_LH.peak_pos == min_max[0]:  # peak on the left
            ...
            rate_contribution = self.date_uncertainty_due_to_rate(
                node, ((1 - fraction) * 0.5, 1.0 - (1.0 - fraction) * 0.5)
            )
            ...
            if node.marginal_pos_LH.y.shape[0] < 3 or pidx == 0 or pidx == node.marginal_pos_LH.y.shape[0] - 1:
                ...
                left = interp1d(
                    node.marginal_pos_LH.y[: (pidx + 1)] - pval,
                    node.marginal_pos_LH.x[: (pidx + 1)],
                    kind='linear',
                    fill_value=min_max[0],
                    bounds_error=False,
                )
                right = interp1d(
                    node.marginal_pos_LH.y[pidx:] - pval,
                    node.marginal_pos_LH.x[pidx:],
                    kind='linear',
                    fill_value=min_max[1],
                    bounds_error=False,
                )
                ...
                if sol['success']:
                    mutation_contribution = self.date2dist.to_numdate(
                        np.array([right(sol['x']), left(sol['x'])]).squeeze()
                    )
