    ...
    def _ml_t_root_leaves(self):
        ...
        for node in self.tree.find_clades(order='preorder'):  # ancestors first, msg to children
            ...
            if node.up is None:
                ...
                res = NodeInterpolator.convolve(msg_parent_to_node, node.branch_length_interpolator,inverse_time=False, n_integral=100)
                ...
                self.logger('ClockTree._ml_t_root_to_leaves: computed convolution'
                            ' with %d points at node %s, sign changes %d'%(len(res.x),node.name, nsign_changed),4)
                node.msg_from_parent = res
                if nsign_changed>1:
                    import matplotlib.pyplot as plt
                    plt.ion()
                    plt.plot(res.x, res.y-res.peak_val)
                    plt.ylim(0,100)
                    plt.xlim(-0.01, 0.01)
