    ...
    def evolve(self, root_seq=None):
        ...
        if root_seq:
            ...
            if len(self.gtr.Pi.shape) == 2:
                ...
                self.tree.root.ancestral_sequence = self.sample_from_profile(
                    np.repeat([self.gtr.Pi], self.seq_len, axis=0)
                )
    ...
    def get_aln(self, internal=False):
        ...
        for n in self.tree.find_clades():
            if n.is_terminal() or internal:
                tmp.append(
                    SeqRecord.SeqRecord(
                        id=n.name, name=n.name, description='', seq=Seq.Seq(''.join(n.ancestral_sequence.astype('U')))
                    )
                )
