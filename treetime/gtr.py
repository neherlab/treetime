...
class GTR(object):
                ...
                while True:
                    ...
                    if line.strip().startswith('Substitution rate (mu):'):
                        ...
                        while line.strip() != '':
                            ...
                            line = f.readline()
                        if not np.any(
                            [len(alphabet) == len(a) and np.all(np.array(alphabet) == a) for a in alphabets.values()]
                        ):
                            raise ValueError(
                                'GTR: was unable to read custom GTR model in '
                                + str(gtr_fname)
                                + ' - Alphabet not recognized'
                            )
                        ...
                        if j != n:
                            raise ValueError(
                                'GTR: was unable to read custom GTR model in '
                                + str(gtr_fname)
                                + ' - Number of lines in W matrix does not match alphabet length'
                            )
    ...
    def infer(cls, nij, Ti, root_state, fixed_pi=None, pc=1.0, gap_limit=0.01, **kwargs):
        ...
        while LA.norm(pi_old - pi) > dp and count < Nit:
            ...
            W_ij = (
                (nij + nij.T + 2 * pc_mat)
                / mu
                / (np.outer(pi, Ti) + np.outer(Ti, pi) + ttconf.TINY_NUMBER + 2 * pc_mat)
            )
            ...
            if fixed_pi is None:
                pi = (np.sum(nij + pc_mat, axis=1) + root_state) / (
                    ttconf.TINY_NUMBER + mu * np.dot(W_ij, Ti) + root_state.sum() + np.sum(pc_mat, axis=1)
                )
                pi /= pi.sum()
        ...
        if gtr.gap_index is not None:
            if pi[gtr.gap_index] < gap_limit:
                gtr.logger(
                    'The model allows for gaps which are estimated to occur at a low fraction of %1.3e'
                    % pi[gtr.gap_index]
                    + ' this can potentially result in artificats.'
                    + ' gap fraction will be set to %1.4f' % gap_limit,
                    2,
                    warn=True,
                )
    ...
    def state_pair(self, seq_p, seq_ch, pattern_multiplicity=None, ignore_gaps=False):
        ...
        if len(self.alphabet) < 10:  # for small alphabet, repeatedly check array for all state pairs
            ...
            for seq in [
                seq_p,
                seq_ch,
            ]:  # for each sequence (parent and child) construct a numerical sequence [0,5,3,1,2,3...]
        ...
        return (
            np.array([x[0] for x in pair_count], dtype=int),  # [(child_nuc, parent_nuc),()...]
            np.array([x[1] for x in pair_count], dtype=int),
        )  # multiplicity of each parent/child nuc pair
    ...
    def prob_t(self, seq_p, seq_ch, t, pattern_multiplicity=None, return_log=False, ignore_gaps=True):
        ...
        """
        seq_pair, multiplicity = self.state_pair(
            seq_p, seq_ch, pattern_multiplicity=pattern_multiplicity, ignore_gaps=ignore_gaps
        )
    ...
    def optimal_t_compressed(self, seq_pair, multiplicity, profiles=False, tol=1e-10):
        ...
        if profiles:
            hamming_distance = 1 - np.sum(multiplicity * np.sum(seq_pair[0] * seq_pair[1], axis=1)) / np.sum(
                multiplicity
            )
        else:
        ...
        try:
            ...
            if 'success' not in opt:
                opt['success'] = True
                self.logger(
                    "WARNING: the optimization result does not contain a 'success' flag:" + str(opt), 4, warn=True
                )
        except ImportError:
            import scipy
        ...
        if new_len > 0.9 * ttconf.MAX_BRANCH_LENGTH:
    ...
    def prob_t_profiles(self, profile_pair, multiplicity, t, return_log=False, ignore_gaps=True):
        ...
        if t < 0:
            ...
            if ignore_gaps and (
                self.gap_index is not None
            ):  # calculate the probability that neither outgroup/node has a gap
    ...
    def _exp_lt(self, t):
        ...
        if any(i > 10 for i in log_val):
            raise ValueError(
                'Error in computing exp(Q * t): Q has positive eigenvalues or the branch length t is too large. '
    ...
    def sequence_logLH(self, seq, pattern_multiplicity=None):
        ...
        return np.sum(
            [
                np.sum((seq == state) * pattern_multiplicity * np.log(self.Pi[si]))
