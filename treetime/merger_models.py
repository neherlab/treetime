...
class Coalescent(object):
    ...
    def set_Tc(self, Tc, T=None):
        ...
        if isinstance(Tc, Iterable):
            ...
            self.Tc = interp1d(
                [-ttconf.BIG_NUMBER, ttconf.BIG_NUMBER], [Tc + ttconf.TINY_NUMBER, Tc + ttconf.TINY_NUMBER]
            )
    ...
    def calc_branch_count(self, posterior=False):
        ...
        self.tree_events = sorted(
            [(n.time_before_present, len(n.clades) - 1) for n in self.tree.find_clades() if not n.bad_branch],
            key=lambda x: -x[0],
        )
    ...
    def calc_integral_merger_rate(self):
        ...
        self.integral_merger_rate = interp1d(
            np.concatenate(([-ttconf.BIG_NUMBER], tvals, [ttconf.BIG_NUMBER])),
            np.concatenate(([cost[0]], cost, [cost[-1]])),
            kind='linear',
        )
    ...
    def cost(self, t_node, branch_length, multiplicity=2.0):
        ...
        return (
            self.integral_merger_rate(merger_time)
            - self.integral_merger_rate(t_node)
    ...
    def optimize_Tc(self):
        ...
        if 'success' in sol and sol['success']:
            ...
            self.logger(
                'merger_models:optimize_Tc: optimization of coalescent time scale failed: ' + str(sol), 0, warn=True
            )
            self.set_Tc(initial_Tc.y, T=initial_Tc.x)

    def optimize_skyline(self, n_points=20, stiffness=2.0, method='SLSQP', tol=0.03, regularization=10.0, **kwarks):
        """
        ...
        def cost(logTc):
            ...
            neglogLH = (
                -self.total_LH()
                + stiffness * np.sum(np.diff(logTc) ** 2)
    ...
    def skyline_empirical(self, gen=1.0, n_points=20):
        ...
        mid_points = np.concatenate(
            ([et[0] - 0.5 * (et[1] - et[0])], 0.5 * (et[1:] + et[:-1]), [et[-1] + 0.5 * (et[-1] - et[-2])])
        )
        ...
        self.Tc_inv = interp1d(
            mid_points[n_points:-n_points],
            [
    ...
    def skyline_inferred(self, gen=1.0, confidence=False):
        ...
        skyline = interp1d(
            self.date2dist.to_numdate(self.Tc.x[1:-1]), gen / self.date2dist.clock_rate * self.Tc.y[1:-1]
        )
