...
def mutations(node):
    ...
    if node.up is None:
        ...
        return node.tt.data.differences(
            node.up.cseq, node.tt.data.aln[node.name], seq2_compressed=False, mask=node.mask
        )
...
class TreeAnc(object):
    ...
    def __init__(
        ...
        fill_overhangs=True,
        ref=None,
        verbose=ttconf.VERBOSE,
        ignore_gaps=True,
        convert_upper=True,
        seq_multiplicity=None,
        log=None,
        compress=True,
        seq_len=None,
    ...
    def tree(self, in_tree):
        ...
        if self._tree.count_terminals() < 3:
            raise MissingDataError(
                'TreeAnc: tree in %s as only %d tips. Please check your tree!'
                % (str(in_tree), self._tree.count_terminals())
            )
        ...
        if branch_length_warning:
            self.logger(
                'WARNING: TreeTime has detected branches that are longer than %d. '
                'TreeTime requires trees where branch length is in units of average number '
                'of nucleotide or protein substitutions per site. '
                'Use on trees with longer branches is not recommended for ancestral sequence reconstruction.'
                % (ttconf.MAX_BRANCH_LENGTH),
                0,
                warn=True,
            )
        self.prepare_tree()
    ...
    def _check_alignment_tree_gtr_consistency(self):
        ...
        if len(self.tree.get_terminals()) != len(self.data.aln):
            self.logger(
                f'**WARNING: Number of tips in tree ({len(self.tree.get_terminals())}) differs from number of sequences in alignment ({len(self.data.aln)})**',
                3,
                warn=True,
            )
        failed_leaves = 0
        ...
        for l in self.tree.find_clades():
            ...
            if l.name not in self.data.compressed_alignment and l.is_terminal():
                self.logger(
                    "***WARNING: TreeAnc._check_alignment_tree_gtr_consistency: NO SEQUENCE FOR LEAF: '%s'" % l.name,
                    0,
                    warn=True,
                )
                failed_leaves += 1
                if not self.ignore_missing_alns and failed_leaves > self.tree.count_terminals() / 3:
                    raise MissingDataError(
                        'TreeAnc._check_alignment_tree_gtr_consistency: At least 30\\% terminal nodes cannot be assigned a sequence!\n'
        ...
        if failed_leaves:
            self.logger(
                "***WARNING: TreeAnc: %d nodes don't have a matching sequence in the alignment."
                ' POSSIBLE ERROR.' % failed_leaves,
                0,
                warn=True,
            )
    ...
    def infer_ancestral_sequences(
        self, method='probabilistic', infer_gtr=False, marginal=False, reconstruct_tip_states=False, **kwargs
    ):
        ...
        **kwargs
        ...
        if method.lower() in ['ml', 'probabilistic']:
            ...
            raise UnknownMethodError(
                "Reconstruction method needs to be in ['ml', 'probabilistic', 'fitch', 'parsimony'], got '{}'".format(
                    method
                )
            )
    ...
    def _fitch_anc(self, **kwargs):
        ...
        if len(ambs) > 0:
            for amb in ambs:
                self.logger(
                    'Ambiguous state of the root sequence '
                    'in the position %d: %s, '
                    'choosing %s' % (amb, str(self.tree.root.state[amb]), self.tree.root.state[amb][0]),
                    4,
                )
        ...
        for node in self.tree.get_nonterminals(order='preorder'):
            if node.up != None:  # not root
                sequence = np.array(
                    [node.up._cseq[i] if node.up._cseq[i] in node.state[i] else node.state[i][0] for i in range(L)]
                )
    ...
    def ancestral_likelihood(self):
        ...
        for node in self.tree.find_clades(order='postorder'):
            ...
            indices = np.array(
                [(self.gtr.state_index[a], self.gtr.state_index[b]) for a, b in zip(node.up.cseq, node.cseq)]
            )
    ...
    def _ml_anc_marginal(self, sample_from_profile=False, reconstruct_tip_states=False, debug=False, **kwargs):
        """
    ...
    def total_LH_and_root_sequence(self, sample_from_profile=False, assign_sequence=False):
        ...
        self.tree.root.marginal_profile, pre = normalize_profile(
            self.tree.root.marginal_outgroup_LH * self.tree.root.marginal_subtree_LH
        )
        ...
        if assign_sequence:
            seq, prof_vals, idxs = prof2seq(
                self.tree.root.marginal_profile,
                self.gtr,
    ...
    def postorder_traversal_marginal(self):
        ...
        for node in self.tree.get_nonterminals(order='postorder'):  # leaves -> root
            ...
            for ch in node.clades:
                if ch.mask is None:
                    ch.marginal_log_Lx = self.gtr.propagate_profile(
                        ch.marginal_subtree_LH, self._branch_length_to_gtr(ch), return_log=True
                    )  # raw prob to transfer prob up
                else:
                    ch.marginal_log_Lx = (
                        self.gtr.propagate_profile(
                            ch.marginal_subtree_LH, self._branch_length_to_gtr(ch), return_log=True
                        ).T
                        * ch.mask
                    ).T  # raw prob to transfer prob up
    ...
    def preorder_traversal_marginal(
        self, reconstruct_tip_states=False, sample_from_profile=False, assign_sequence=False
    ):
        ...
        for node in self.tree.find_clades(order='preorder'):
            ...
            node.marginal_outgroup_LH, pre = normalize_profile(
                np.log(np.maximum(ttconf.TINY_NUMBER, node.up.marginal_profile)) - node.marginal_log_Lx,
                log=True,
                return_offset=False,
            )
            if node.is_terminal() and (
                not reconstruct_tip_states
            ):  # skip remainder unless leaves are to be reconstructed
                continue

            tmp_msg_from_parent = self.gtr.evolve(
                node.marginal_outgroup_LH, self._branch_length_to_gtr(node), return_log=False
            )

            if node.mask is None:
                node.marginal_profile, pre = normalize_profile(
                    node.marginal_subtree_LH * tmp_msg_from_parent, return_offset=False
                )
            else:
                node.marginal_profile, pre = normalize_profile(
                    node.marginal_subtree_LH * (node.mask * tmp_msg_from_parent.T + (1.0 - node.mask)).T,
                    return_offset=False,
                )
            ...
            if assign_sequence:
                seq, prof_vals, idxs = prof2seq(
                    node.marginal_profile, self.gtr, sample_from_prof=sample_from_profile, normalize=False, rng=self.rng
                )
    ...
    def _ml_anc_joint(self, sample_from_profile=False, reconstruct_tip_states=False, debug=False, **kwargs):
        """
        ...
        for node in self.tree.find_clades(order='postorder'):
            ...
            for char_i, char in enumerate(self.gtr.alphabet):
                ...
                if node.mask is None:
                    ...
                    msg_to_parent = (
                        log_transitions[:, char_i] * np.repeat([node.mask], self.gtr.n_states, axis=0).T
                    ) + msg_from_children
    ...
    def optimize_branch_lengths_joint(self, **kwargs):
        ...
        **Note** this method assumes that each node stores information
        ...
        self.logger(
            'TreeAnc.optimize_branch_length: running branch length optimization using jointML ancestral sequences', 1
        )
        ...
        for node in self.tree.find_clades(order='postorder'):
            ...
            self.logger(
                'Optimization results: old_len=%.4e, new_len=%.4e'
    ...
    def optimal_branch_length(self, node):
        ...
        if not hasattr(node, 'branch_state'):
            if node.cseq is None and node.is_terminal():
                raise MissingDataError(
                    "TreeAnc.optimal_branch_length: terminal node alignments required; sequence is missing for leaf: '%s'. "
                    'Missing terminal sequences can be inferred from sister nodes by rerunning with `reconstruct_tip_states=True` or `--reconstruct-tip-states`'
                    % node.name
                )
            self.add_branch_state(node)
        return self.gtr.optimal_t_compressed(node.branch_state['pair'], node.branch_state['multiplicity'])
    ...
    def optimal_marginal_branch_length(self, node, tol=1e-10):
        ...
        if node.up is None:
            ...
            pp, pc = self.marginal_branch_profile(node)
            return self.gtr.optimal_t_compressed(
                (pp, pc), self.data.multiplicity(mask=node.mask), profiles=True, tol=tol
            )

    def optimize_tree_marginal(
        self, max_iter=10, infer_gtr=False, pc=1.0, damping=0.75, LHtol=0.1, site_specific_gtr=False, **kwargs
    ):
        self.infer_ancestral_sequences(marginal=True, **kwargs)
        oldLH = self.sequence_LH()
        self.logger(
            'TreeAnc.optimize_tree_marginal: initial, LH=%1.2f, total branch_length %1.4f'
            % (oldLH, self.tree.total_branch_length()),
            2,
        )
        for i in range(max_iter):
            ...
            for n in self.tree.find_clades():
                ...
                if n.up.up is None and len(n.up.clades) == 2:
                    ...
                    if n1.mask is None or n2.mask is None:
                        new_bl = self.gtr.optimal_t_compressed(
                            (prof_p, prof_c), self.data.multiplicity(), profiles=True, tol=tol
                        )
                    else:
                        new_bl = self.gtr.optimal_t_compressed(
                            (prof_p, prof_c), self.data.multiplicity(mask=n1.mask * n2.mask), profiles=True, tol=tol
                        )
            ...
            self.logger(
                'TreeAnc.optimize_tree_marginal: iteration %d, LH=%1.2f (%1.2f), delta branch_length=%1.4f, total branch_length %1.4f'
                % (i, LH, deltaLH, dbl, self.tree.total_branch_length()),
                2,
            )
            if deltaLH < LHtol:
    ...
    def optimize_sequences_and_branch_length(self, *args, **kwargs):
        ...
        """
        self.logger(
            "Deprecation warning: 'optimize_sequences_and_branch_length' will be removed and replaced by 'optimize_tree'!",
            1,
            warn=True,
        )
    ...
    def optimize_seq_and_branch_len(self, *args, **kwargs):
        ...
        """
        self.logger(
            "Deprecation warning: 'optimize_seq_and_branch_len' will be removed and replaced by 'optimize_tree'!",
            1,
            warn=True,
        )
        self.optimize_tree(*args, **kwargs)
    ...
    def optimize_tree(
        ...
        **kwargs,
        ...
        if branch_length_mode == 'marginal':
            ...
            N_diff = self.reconstruct_anc(
                method=method_anc, infer_gtr=infer_gtr, pc=pc, marginal=marginal_sequences, **kwargs
            )
            if prune_short:
                self.prune_short_branches()
            return ttconf.SUCCESS
        ...
        while n < max_iter:
            ...
            if prune_short:
                self.prune_short_branches()
            N_diff = self.reconstruct_anc(method=method_anc, infer_gtr=False, marginal=marginal_sequences, **kwargs)

            self.logger(
                'TreeAnc.optimize_tree: Iteration %d. #Nuc changed since prev reconstructions: %d' % (n, N_diff), 2
            )
    ...
    def prune_short_branches(self):
        ...
        for node in self.tree.find_clades():
            ...
            if (node.branch_length < 0.1 * self.one_mutation) and (
                self.gtr.prob_t(
                    node.up._cseq, node._cseq, 0.0, pattern_multiplicity=self.data.multiplicity(mask=node.mask)
                )
                > 0.1
            ):
    ...
    def infer_gtr(self, marginal=False, site_specific=False, normalized_rate=True, fixed_pi=None, pc=5.0, **kwargs):
        ...
        if site_specific:
            ...
            self._gtr = GTR_site_specific.infer(
                n_ija,
                T_ia,
                pc=pc,
    ...
    def infer_gtr_iterative(self, max_iter=10, site_specific=False, LHtol=0.1, pc=1.0, normalized_rate=False):
        ...
        for i in range(max_iter):
            self.infer_gtr(site_specific=site_specific, marginal=True, normalized_rate=normalized_rate, pc=pc)
            self.infer_ancestral_sequences(marginal=True)
            ...
            self.logger(
                'TreeAnc.infer_gtr_iterative: iteration %d, LH=%1.2f (%1.2f), deltaP=%1.4f' % (i, old_LH, deltaLH, dp),
                2,
            )
            if deltaLH < LHtol:
    ...
    def optimize_gtr_rate(self):
        ...
        def cost_func(sqrt_mu):
            ...
            self.postorder_traversal_marginal()
            self.total_LH_and_root_sequence(sample_from_profile=False, assign_sequence=False)
            return -self.sequence_LH()

        old_mu = self.gtr.mu
        try:
            sol = minimize_scalar(
                cost_func, bracket=[0.01 * np.sqrt(old_mu), np.sqrt(old_mu), 100 * np.sqrt(old_mu)], method='brent'
            )
        except:
            self.gtr.mu = old_mu
        ...
        if sol['success']:
            self.gtr.mu = sol['x'] ** 2
            self.logger(
                'treeanc:optimize_gtr_rate: optimization successful. Overall rate estimated to be %f' % self.gtr.mu, 1
            )
        else:
            self.gtr.mu = old_mu
            self.logger('treeanc:optimize_gtr_rate: optimization failed, continuing with previous mu', 1, warn=True)
    ...
    def get_reconstructed_alignment(self, reconstruct_tip_states=False):
        ...
        if self.data.is_sparse:
            new_aln = {
                'sequences': {n.name: self.data.compressed_to_sparse_sequence(n.cseq) for n in self.tree.find_clades()}
            }
            ...
            new_aln = MultipleSeqAlignment(
                [
                    SeqRecord(
                        id=n.name,
                        seq=Seq(
                            self.sequence(n, reconstructed=reconstruct_tip_states, as_string=True, compressed=False)
                        ),
                        description='',
                    )
                    for n in self.tree.find_clades()
    ...
    def sequence(self, node, reconstructed=False, as_string=True, compressed=False):
        ...
        if type(node) == str:
            if node in self.leaves_lookup:
                ...
                raise ValueError(
                    'TreeAnc.sequence accepts strings are argument only when the node is terminal and present in the leave lookup table'
                )

        if reconstructed and not self.reconstructed_tip_sequences:
            raise ValueError(
                'TreeAnc.sequence can only return reconstructed terminal nodes if TreeAnc.infer_ancestral_sequences was run with this the flag `reconstruct_tip_states`.'
            )
    ...
    def recover_var_ambigs(self):
        self.logger(
            'TreeAnc: recover_var_ambigs: calls to recover_var_ambigs are no longer necessary since tip states are not inferred unless explicitly specified using `reconstruct_tip_states=True`.',
            0,
            warn=True,
        )
        if self.reconstructed_tip_sequences:
            self.logger(
                'Your code reconstructed tip states, please change the call of ancestral inference in your code',
                0,
                warn=True,
            )
        else:
            self.logger(
                'Your analysis did not reconstructed tip states, you can remove the call of `recover_var_ambigs`',
                0,
                warn=True,
            )
