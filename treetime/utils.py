...
def min_interp(interp_object):
    ...
    try:
        ...
        s = (
            ...
            + str(interp_object.x)
            + 'Minimal x: '
            + str(interp_object.x.min())
            + 'Maximal x: '
            + str(interp_object.x.max())
        )
        raise e
...
def median_interp(interp_object):
    ...
    """
    new_grid = np.sort(
        np.concatenate([interp_object.x[:-1] + 0.1 * ii * np.diff(interp_object.x) for ii in range(10)]).flatten()
    )
    ...
    for val, field in zip(mydate.split(sep), fmt.split(sep + '%')):
        ...
        if 'XX' in val:
            ...
            try:
                ...
                return None, None
    max_date['day'] = min(
        max_date['day'], 31 if max_date['month'] in [1, 3, 5, 7, 8, 10, 12] else 28 if max_date['month'] == 2 else 30
    )
...
def tree_inference(aln_fname, tree_fname, tmp_dir=None, methods=None, **kwargs):
    import os, shutil
...
def build_newick_iqtree(aln_fname, nthreads=2, iqtree_bin='iqtree', iqmodel='HKY', **kwargs):
    import os
    from Bio import Phylo, AlignIO
    ...
    fast_opts = ['-ninit', '2', '-n', '2', '-me', '0.05']

    call = ['iqtree'] + fast_opts + ['-nt', str(nthreads), '-s', aln_file, '-m', iqmodel, '>', 'iqtree.log']
