...
class Distribution(object):
    ...
    def calc_fwhm(distribution, is_neg_log=True):
        ...
        if len(tmp) == 0:
            raise ValueError(
                'Error in computing the FWHM for the distribution. This is most likely caused by incorrect input data.'
            )
    ...
    @classmethod
    def delta_function(cls, x_pos, weight=1.0, min_width=MIN_INTEGRATION_PEAK):
        """
        Create delta function distribution.
        """

        distribution = cls(x_pos, 0.0, is_log=True, min_width=min_width)
        ...
        if n_delta > 1:
            ...
            try:
                ...
                raise TreeTimeUnknownError(
                    'Error: Unexpected behavior detected in multiply function'
            ...
            if n_points == 0:
                ...
                res = Distribution(
                    x_vals[ind], y_vals[ind], is_log=True, min_width=min_width, kind='linear', assume_sorted=True
                )
    ...
    def divide(numerator, denominator):
        ...
        if n_points == 0:
            print(
                'WARNING: Unexpected behavior detected in multiply function,'
    ...
    def __init__(self, x, y, is_log=True, min_width=MIN_INTEGRATION_PEAK, kind='linear', assume_sorted=False):
        ...
        if isinstance(x, Iterable) and isinstance(y, Iterable):
            ...
            self._func = interp1d(
                xvals, yvals, kind=kind, fill_value=BIG_NUMBER, bounds_error=False, assume_sorted=True
            )
            ...
            raise TypeError('Cannot create Distribution: Input arguments should be scalars or iterables!')
    ...
    def __call__(self, x):
        if isinstance(x, Iterable):
            ...
            tmp_x = x[valid_idxs]
            res[valid_idxs] = self._peak_val + self._func(
                clip(tmp_x, self._xmin + TINY_NUMBER, self._xmax - TINY_NUMBER)
            )
            return res
    ...
    def integrate_simpson(self, a=None, b=None, n=None):
        ...
        for lw, up in zip(threshold[:-1], threshold[1:]):
            ...
            res.append(
                mult
                * (dx[0] * y[0] + np.sum(4 * dx * y[1:-1:2]) + np.sum((dx[:-1] + dx[1:]) * y[2:-1:2]) + dx[-1] * y[-1])
            )
