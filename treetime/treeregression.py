...
def base_regression(Q, slope=None):
    ...
    if (Q[tsqii] - Q[tavgii] ** 2 / Q[sii]) > 0:
        chisq = 0.5 * (
            Q[dsqii]
            - Q[davgii] ** 2 / Q[sii]
            - (Q[dtavgii] - Q[davgii] * Q[tavgii] / Q[sii]) ** 2 / (Q[tsqii] - Q[tavgii] ** 2 / Q[sii])
        )
    else:
        chisq = 0.5 * (Q[dsqii] - Q[davgii] ** 2 / Q[sii])

    if only_intercept:
        return {'slope': slope, 'intercept': intercept, 'chisq': chisq}
...
class TreeRegression(object):
    ...
    def __init__(self, tree_in, tip_value=None, branch_value=None, branch_variance=None):
        """
        Parameters
        ----------
         T : (Bio.Phylo.tree)
        ...
        for n in self.tree.get_nonterminals(order='postorder'):
            ...
            for c in n:
                ...
                if c.is_terminal():
                    ...
                    if full_matrix:
                        M[c_count : c_count + nc, c_count : c_count + nc] = c.cinv - ssq * np.outer(c.r, c.r) / (
                            1 + ssq * c.s
                        )
        ...
        if n.is_terminal() and outgroup == False:
            ...
            res = np.array(
                [
                    ...
                    + 2 * bv * tmpQ[davgii]
                    + bv**2 * tmpQ[sii]
                    - var * (tmpQ[davgii] ** 2 + 2 * bv * tmpQ[davgii] * tmpQ[sii] + bv**2 * tmpQ[sii] ** 2) * denom,
                    tmpQ[sii] * denom,
                ]
            )
    ...
    def find_best_root(self, force_positive=True, slope=None):
        ...
        for n in self.tree.find_clades():
            ...
            if chisq < best_root['chisq']:
                tmpQ = self.propagate_averages(n, tv, bv * x, var * x) + self.propagate_averages(
                    n, tv, bv * (1 - x), var * (1 - x), outgroup=True
                )
        ...
        if 'hessian' in best_root:
            ...
            for dx in [-0.001, 0.001]:
                ...
                tmpQ = self.propagate_averages(n, tv, bv * y, var * y) + self.propagate_averages(
                    n, tv, bv * (1 - y), var * (1 - y), outgroup=True
                )
                reg = base_regression(tmpQ, slope=slope)
            ...
            estimator_hessian[2, 2] = (deriv[0][1] + deriv[1][1] - 2.0 * best_root['chisq']) / (
                deriv[0][0] - deriv[1][0]
            ) ** 2
    ...
    def optimal_reroot(self, force_positive=True, slope=None, keep_node_order=False):
        ...
        if x < 1e-5:
            ...
            new_node.up.clades = [k if k != best_node else new_node for k in best_node.up.clades]
    ...
    def clock_plot(self, add_internal=False, ax=None, regression=None, confidence=True, n_sigma=2, fs=14):
        ...
        if regression:
            ...
            if add_internal:
                ...
                x_vals = np.array(
                    [max(np.min(xi_int[~ind_int]), t_mrca) - 0.1 * time_span, np.max(xi[~ind]) + 0.05 * time_span]
                )
            else:
                time_span = np.max(xi[~ind]) - np.min(xi[~ind])
                x_vals = np.array(
                    [max(np.min(xi[~ind]), t_mrca) - 0.1 * time_span, np.max(xi[~ind] + 0.05 * time_span)]
                )
            ...
            if confidence and 'cov' in regression:
                ...
                dev = n_sigma * np.array(
                    [np.sqrt(regression['cov'][:2, :2].dot(np.array([x, 1])).dot(np.array([x, 1]))) for x in x_vals]
                )
        ...
        if ind.sum():
            try:
                ...
                tmp_x = np.array(
                    [
                        np.mean(n.raw_date_constraint) if n.raw_date_constraint else None
