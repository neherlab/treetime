...
class GTR_site_specific(GTR):
    ...
    def assign_rates(self, mu=1.0, pi=None, W=None):
        ...
        if not np.isscalar(mu) and pi is not None and len(pi.shape) == 2:
            if mu.shape[0] != pi.shape[1]:
                raise ValueError(
                    'GTR_site_specific: length of rate vector (got {}) and equilibrium frequency vector (got {}) must match!'.format(
                        mu.shape[0], pi.shape[1]
                    )
                )
        ...
        if pi is not None and pi.shape[0] == n and len(pi.shape) == 2:
            ...
            if pi is not None:
                if len(pi) == n:
                    ...
                    raise ValueError(
                        'GTR_site_specific: length of equilibrium frequency vector (got {}) does not match alphabet length {}'.format(
                            len(pi), n
                        )
                    )
            else:
        ...
        if W is None or W.shape != (n, n):
            if (W is not None) and W.shape != (n, n):
                raise ValueError(
                    'GTR_site_specific: Size of substitution matrix (got {}) does not match alphabet length {}'.format(
                        W.shape, n
                    )
                )
            W = np.ones((n, n))
    ...
    @classmethod
    def random(
        cls, L=1, avg_mu=1.0, alphabet='nuc', pi_dirichlet_alpha=1, W_dirichlet_alpha=3.0, mu_gamma_alpha=3.0, rng=None
    ):
        """
    ...
    @classmethod
    def infer(cls, sub_ija, T_ia, root_state, pc=1.0, gap_limit=0.01, Nit=30, dp=1e-5, **kwargs):
        r"""
    ...
    def _make_expQt_interpolator(self):
        ...
        t_grid = (1.0 / self.rate_scale) * np.concatenate(
            (
                np.linspace(0, 0.1, 11)[:-1],
        ...
        self.expQt_interpolator = interp1d(t_grid, stacked_expQT, axis=0, assume_sorted=True, copy=False, kind='linear')
