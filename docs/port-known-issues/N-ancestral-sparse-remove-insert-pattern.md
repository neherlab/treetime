...
The dense forward pass had the same pattern and was fixed (commit `052e6fcd`) by replacing remove/insert with shared borrows + `get_mut`, exploiting disjoint field borrows of `partition.nodes` and `partition.edges`.
...
- `packages/treetime/src/representation/partition/marginal_passes.rs:189:` - `process_node_forward` child loop: edge remove/insert. Structurally identical to the dense case that was fixed. Disjoint field borrows (`&partition.nodes` + `&mut partition.edges`) apply directly.
