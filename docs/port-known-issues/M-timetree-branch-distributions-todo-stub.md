# build_branch_distributions is a public todo!() stub
...
Either remove `build_branch_distributions` and the `Joint` variant entirely, or replace the `todo!()` with the actual implementation if the function is needed. If kept, remove the `Joint` variant to match the v1 design decision documented in [port-intentional-changes/ancestral-joint-reconstruction-removed.md](../port-intentional-changes/ancestral-joint-reconstruction-removed.md).

## Related

- [port-intentional-changes/ancestral-joint-reconstruction-removed.md](../port-intentional-changes/ancestral-joint-reconstruction-removed.md): joint reconstruction removal
- [N-timetree-n-branches-posterior-unimplemented.md](N-timetree-n-branches-posterior-unimplemented.md): related unimplemented branch distribution feature
