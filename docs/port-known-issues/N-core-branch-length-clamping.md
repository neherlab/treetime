...
[`fix_branch_length()`](../../packages/treetime/src/hacks/fix_branch_length.rs#L6)
(`#fix_branch_length`) clamps branch lengths to
`MIN_BRANCH_LENGTH_FRACTION * (1/L)` to avoid NaN in the marginal backward
pass, which computes `log(expQt)` - divergent at `t=0`. Mathematically,
`expQt(0) = I` is correct (identity matrix), but the log-space computation path
requires `t > 0`.
