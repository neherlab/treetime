...
The root cause is not fully understood. v1 compensates by using `force_positive_rate: false` for the pre-filter step (documented in `port-intentional-changes/clock-prefilter-relaxed-positive-rate.md`). This works but means v1's pre-filter uses a different root than v0 would choose, contributing to different outlier sets.
