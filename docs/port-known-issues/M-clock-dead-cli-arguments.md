...
For each flag, either wire it to the appropriate behavior or remove it from the CLI surface. If removal is chosen, document the omission in [port-known-issues/](.) or [port-intentional-changes/](../port-intentional-changes/) as appropriate.
