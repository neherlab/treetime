...
Either implement tree inference from alignment (a large feature tracked in [port-algo-inventory/unimplemented.md](../port-algo-inventory/unimplemented.md)) or replace the `todo!()` with a descriptive error: "Tree inference from alignment is not yet implemented. Provide a tree with --tree."
