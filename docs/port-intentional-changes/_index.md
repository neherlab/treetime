...
| Optimize   | [Dense initial guess: soft Hamming on per-edge messages](optimize-dense-initial-guess-soft-hamming.md#dense-initial-branch-length-guess-uses-soft-hamming-on-per-edge-messages) | Initial branch length estimate for dense mode  |
