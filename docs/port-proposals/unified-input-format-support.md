...
Analysis commands (`ancestral`, `timetree`, `optimize`, `clock`, `mugration`) currently accept only Newick trees and FASTA alignments as separate files. This two-file model requires name-based matching to reconcile sequences with tree nodes, which is unreliable and inefficient (see [M-io-sequence-name-matching-unreliable](../port-known-issues/M-io-sequence-name-matching-unreliable.md) and [M-io-sequence-attachment-quadratic](../port-known-issues/M-io-sequence-attachment-quadratic.md)).
...
- [Multi-format tree I/O](../port-intentional-changes/multi-format-tree-io.md): documents the existing format adapters that this proposal would integrate
...
- [Sequence-to-node name matching is unreliable](../port-known-issues/M-io-sequence-name-matching-unreliable.md)
- [Sequence attachment has O(n squared) complexity](../port-known-issues/M-io-sequence-attachment-quadratic.md)
- [Large datasets require all sequences in memory](../port-known-issues/N-io-large-dataset-memory-constraint.md)
- [Auspice JSON output incomplete](../port-known-issues/N-timetree-auspice-json-incomplete.md)
- [Multi-segment genome input not wired](../port-known-issues/N-io-multi-segment-genome-input.md)

## Related documentation

- [Multi-format tree I/O](../port-intentional-changes/multi-format-tree-io.md) - existing format adapter implementations
- [Partition system architecture](../port-intentional-changes/partition-system-architecture.md) - target internal representation
