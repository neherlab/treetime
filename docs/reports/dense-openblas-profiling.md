...
The [developer guide](../dev/developer_guide.md) instructs developers to install `libopenblas-dev`:
...
2. **Developer guide change.** Replace `libopenblas-dev` with `libopenblas-serial-dev` in the [developer guide](../dev/developer_guide.md). This installs the sequential variant system-wide. Developers who already have `libopenblas-dev` installed can switch with:
...
- **Update developer guide** -- replace `libopenblas-dev` with `libopenblas-serial-dev` in the [developer guide](../dev/developer_guide.md) dependency list.
