...
export const noVersionedNamesRule = defineRule({
  meta: {
    ...
    docs: {
      description:
        "Disallow version suffixes such as `_v2`, `_old`, `_new`, and `_tmp` on declared names.",
    },
    messages: {
      versioned:
        "`{{name}}` carries a version suffix. Names must describe the final state, not edit history.",
    },
  },
  createOnce(context) {
