...
function isErrorHandler(node: FunctionNode, sourceCode: SourceCode): boolean | undefined {
  ...
  const position =
    (method === "catch" && parent.arguments[0] === node) ||
    (method === "then" && parent.arguments[1] === node)
...
function isPromise(
  ...
  if (callee.type === "Identifier") {
    ...
    const fn =
      definition?.type === "FunctionName" ? definition.node : bindingInitializer(callee, sourceCode)
...
function successReturns(
  ...
  return new Set(
    paths.flatMap((path) =>
      path.kind === "return" &&
      !path.handled &&
      resultOwner(path.node) === analysis.owner &&
      isSuccessValue(path.value)
        ? [path.node]
        : [],
    ),
  )
...
): ESTree.Expression | ESTree.BlockStatement | ESTree.Program {
...
function statementCompletions(
  ...
  if (statement.type === "ThrowStatement") {
    return evaluationCompletions(statement.argument, handled, analysis, "propagate").map(
      (path) => ({ kind: "throw", handled: path.handled }),
    )
  }
  ...
  if (statement.type === "IfStatement") {
    ...
    return continueCompletions(
      evaluationCompletions(statement.test, handled, analysis),
      (observed) => {
        ...
        return [
          ...(truth === false
            ? []
            : statementCompletions(statement.consequent, observed, analysis)),
  ...
  if (statement.type === "LabeledStatement") {
    return statementCompletions(statement.body, handled, analysis).map((path): Completion =>
      path.kind === "break" && path.label === statement.label.name
        ? { kind: "normal", handled: path.handled }
        : path,
    )
  }
...
function continueCompletions(
  paths: Completion[],
  next: (handled: boolean) => Completion[],
): Completion[] {
  return mergeCompletions(
    paths.flatMap((path) => (path.kind === "normal" ? next(path.handled) : [path])),
  )
}
...
function declarationCompletions(
  ...
  for (const declaration of statement.declarations) {
    paths = continueCompletions(paths, (observed) =>
      continueCompletions(
        evaluationCompletions(declaration.init ?? undefined, observed, analysis),
        (initialized) => patternCompletions(declaration.id, initialized, analysis),
      ),
    )
  }
  return paths
...
function catchCompletions(
  ...
  return continueCompletions(entry, (observed) =>
    statementCompletions(handler.body, observed, analysis),
  )
}
  ...
  return continueCompletions(entry, (initialized) => {
    ...
    while (pending.length > 0) {
      ...
      for (const path of start) {
        ...
        const body = continueCompletions(
          loopBindingCompletions(statement, path.handled, analysis),
          (bound) => statementCompletions(statement.body, bound, analysis),
        )
...
function loopBindingCompletions(
  statement: LoopStatement,
  handled: boolean,
  analysis: ReturnAnalysis,
): Completion[] {
...
function loopAdvanceCompletions(
  statement: LoopStatement,
  handled: boolean,
  analysis: ReturnAnalysis,
): Completion[] {
...
function switchCompletions(
  ...
  for (const [index, branch] of statement.cases.entries()) {
    ...
    unmatched = continueCompletions(unmatched, (observed) =>
      evaluationCompletions(test, observed, analysis),
    )
    const matches =
    ...
    if (matches !== false) {
      branches.push(
        ...continueCompletions(unmatched, (observed) =>
          switchBranchCompletions(statement, index, observed, analysis),
        ),
      )
    }
    if (matches === true) {
...
function switchBranchCompletions(
  ...
  return blockCompletions(
    ...
    path.kind === "break" && path.label === undefined
      ? { kind: "normal", handled: path.handled }
      : path,
  )
}
...
function expressionCompletions(
  ...
  if (expression?.type === "ConditionalExpression") {
    ...
    return continueCompletions(
      evaluationCompletions(expression.test, handled, analysis),
      (observed) => [
        ...(truth === false
          ? []
          : expressionCompletions(expression.consequent, node, observed, analysis)),
        ...(truth === true
          ? []
          : expressionCompletions(expression.alternate, node, observed, analysis)),
      ],
    )
  }
  if (expression?.type === "SequenceExpression") {
    ...
    for (const part of expression.expressions.slice(0, -1)) {
      paths = continueCompletions(paths, (observed) =>
        evaluationCompletions(part, observed, analysis),
      )
    }
  ...
  return evaluationCompletions(expression, handled, analysis, "propagate").map(
    (path): Completion =>
      path.kind === "normal"
        ? { kind: "return", node, value: expression, handled: path.handled }
        : path,
  )
}
...
function isSuccessValue(expression: ESTree.Expression | undefined): boolean {
  ...
  if (expression.type === "ObjectExpression") {
    return (
      expression.properties.some(isSuccessStatus) && expression.properties.every(isSuccessProperty)
    )
  }
  return false
}
