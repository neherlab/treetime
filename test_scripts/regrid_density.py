...
`regrid` returns the same kind of object on a coarse grid: it keeps the left
support edge xmin (nothing to truncate there — the density vanishes) and cuts
the exponential tail at the 1-epsilon quantile.  The y values are copied from
...
class LogDensity:
    ...
    def mass_between(self, a, b):
        """Fraction of the total mass on [a, b]."""
...
# ----------------------------------------------------------------------------
def regrid(f: LogDensity, n_points, epsilon=1e-4, epsilon_left=None):
    """Resample `f` onto `n_points` regular points, keeping its policies.

    The new range is [xmin_new, quantile(1 - epsilon)].  By default
    xmin_new = f.xmin, the support edge, where no mass is lost; pass
    `epsilon_left` to cut the left side at that quantile instead.
    ...
    x_lo = f.xmin if epsilon_left is None else f.quantile(epsilon_left)
    x_hi = f.quantile(1.0 - epsilon)
    if not x_hi > x_lo:
        raise ValueError("degenerate range")

    x_new = np.linspace(x_lo, x_hi, n_points)
    y_new = f(x_new)                       # y_new[0] = inf when x_lo == f.xmin
    ...
    # x_hi is epsilon by construction.
    m_edge = f.mass_between(x_lo, x_new[1])
    if not m_edge > 0.0:
        raise ValueError("the first cell of the new grid carries no mass; "
                         "use fewer points or a larger epsilon_left")
...
def diagnostics(before: LogDensity, after: LogDensity):
    ...
    return {
        "mass_covered": f.cdf(g.xmax) - f.cdf(g.xmin),
...
if __name__ == "__main__":
    ...
    for n_new in (9, 17, 33, 65):
        g = regrid(f, n_new, epsilon=1e-6, epsilon_left=1e-6)
        d = diagnostics(f, g)
    ...
    plot(f, regrid(f, 100, epsilon=1e-6, epsilon_left=1e-6), truth=neg_log_p)
    print("\nwrote regrid_density.png")
