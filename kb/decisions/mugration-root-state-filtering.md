...
v1 skips the root state contribution to equilibrium frequency estimation when the root posterior profile is near-uniform at [packages/treetime/src/gtr/refinement.rs#L104-L108](../../packages/treetime/src/gtr/refinement.rs#L104-L108). A root profile with max probability at or below `1/n_states + 1e-10` carries no phylogenetic signal and is excluded.
...
Matches the dense GTR inference path's approach at [packages/treetime/src/ancestral/gtr_inference_dense.rs#L169](../../packages/treetime/src/ancestral/gtr_inference_dense.rs#L169), which already filters uninformative root profiles. Eliminates state-order bias in the `root_state` vector that feeds into `infer_gtr_impl()` pi estimation.
