...
The coalescent factor is then applied by `distribution_apply_neg_log_weight` ([`packages/treetime-distribution/src/distribution_ops/log_cost.rs`](../../packages/treetime-distribution/src/distribution_ops/log_cost.rs)), which shifts by the combined minimum of `contribution(t) - ln(amplitude(t))`. Where the incoming amplitude has already underflowed to 0, `-ln(0)` is `+inf` and the point is dropped; the log-space shift cannot recover mass the plain-space product already lost.
