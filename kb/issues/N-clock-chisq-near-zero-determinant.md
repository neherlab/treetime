...
In practice, with `ClockParams::default()` (zero `variance_factor`), the determinant at internal nodes is `n² * var(dates)`, which is well-conditioned for any dataset with date variation. The risk is theoretical.
