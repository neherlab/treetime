...
- `run_optimize_mixed_with_indel_rate()` at `packages/treetime/src/commands/optimize/optimize_unified.rs:552` is unused in production after the refactor (only called by tests). Gate with `#[cfg(test)]` to eliminate the dead-code warning.
