...
4. Sparse EPS demotion: `combine_messages()` demotes variable sites to fixed
   ...
   The same demotion also drove a reconstruction defect, now fixed and recorded in [The sparse stored sequence is the parsimony chain](../decisions/ancestral-marginal-sparse-parsimony-chain.md). That defect affected emitted sequences only and left log-likelihoods untouched, so it neither explains nor eliminates this lead - the demotion's effect on log-likelihood still needs evaluating on its own.
