...
`BranchLengthMode`, `RerootMode`, `MethodAncestral` extracted to [packages/treetime/src/commands/shared/args.rs](../../packages/treetime/src/commands/shared/args.rs). All cross-command arg imports now go through `commands::shared::args`.

### Output helpers

`write_graph()` duplicated across `ancestral`, `optimize`, `prune`. Already tracked in [L-core-duplicate-write-graph](N-core-duplicate-write-graph.md).
