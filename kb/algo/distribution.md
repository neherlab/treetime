...
- Distribution Division: `distribution_division()` (`#distribution_division`) in [`packages/treetime-distribution/src/distribution_ops/divide.rs#L14`](../../packages/treetime-distribution/src/distribution_ops/divide.rs#L14) - Cavity computation via pointwise division. Range-Function and Function-Function division use the exact analytical support intersection on sides where the divisor has the default `Error` boundary behavior, with exact endpoint contact represented as a Point distribution. An explicit divisor tail extends its side of the division domain to the dividend boundary; [kb/decisions/timetree-inference-pass-boundary-tails.md](../decisions/timetree-inference-pass-boundary-tails.md) defines those tails. Plain division floors small denominators at $10^{-10}$, changing the quotient; Formula division panics. The scientific small-divisor policy is unresolved.
