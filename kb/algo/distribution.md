...
Polymorphic pointwise multiplication dispatches across all `Distribution` variant pairs, including Formula. The implementation silently constructs new uniform grids: Function-Function uses the larger input length over the overlap; Formula-Formula uses 200 points; Formula-Function uses the Function's point count over a new overlap grid. v0 instead uses the union of original coordinates, so this is an unresolved numerical-parity divergence. Mixed-support endpoint, point-intersection, NaN, and negative-roundoff semantics are also unresolved.
...
- Distribution Division: `distribution_division()` (`#distribution_division`) in [`packages/treetime-distribution/src/distribution_ops/divide.rs#L9-L51`](../../packages/treetime-distribution/src/distribution_ops/divide.rs#L9-L51) - Cavity computation via pointwise division. Plain division floors small denominators at $10^{-10}$, changing the quotient; Formula division panics. The scientific divisor policy is unresolved.
