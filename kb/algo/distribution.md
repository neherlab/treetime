...
Polymorphic pointwise multiplication dispatches across all `Distribution` variant pairs, including Formula. Range-Function multiplication uses the exact analytical support intersection and a positive-width grid no coarser than the Function input. Function-Function multiplication also uses exact analytical endpoints. Exact endpoint contact in these two paths produces a Point distribution as in v0. The implementation otherwise silently constructs new uniform grids: Function-Function uses the larger input length over the overlap, Formula-Formula uses 200 points, and Formula-Function uses the Function's point count over a new overlap grid. v0 instead uses the union of original coordinates, so this remains an unresolved numerical-parity divergence. NaN and negative-roundoff semantics are also unresolved.
...
- Distribution Division: `distribution_division()` (`#distribution_division`) in [`packages/treetime-distribution/src/distribution_ops/divide.rs#L9-L51`](../../packages/treetime-distribution/src/distribution_ops/divide.rs#L9-L51) - Cavity computation via pointwise division. Range-Function and Function-Function division use the exact analytical support intersection, with exact endpoint contact represented as a Point distribution. Plain division floors small denominators at $10^{-10}$, changing the quotient; Formula division panics. The scientific divisor policy is unresolved.
