...
Range-Function, Function-Function, and Formula-Function multiplication are defined on the exact intersection of their operand supports. Range-Function and Function-Function division use the same per-side support rule as multiplication (intersect hard sides to the innermost bound, union soft sides to the outermost); the `[max(a.x_min, b.x_min), min(a.x_max, b.x_max)]` form is the both-hard case. An intersection is empty when `overlap_min > overlap_max`; exact endpoint contact produces a Point distribution evaluated at the shared endpoint, matching v0.

Under this rule a soft divisor tail extends the quotient -- the divisor's decaying tail is sampled as bulk out to the union edge -- instead of truncating it, because the cavity dividend is a product that contains the divisor as a factor, so the quotient decays rather than spiking. A divisor `Hard`/`HardApproach`/`Error` edge strictly inside the dividend still bounds the quotient there, and a hard dividend edge bounds it to zero. Left and right sides are resolved independently. [kb/decisions/distribution-tails-and-arithmetic.md](../decisions/distribution-tails-and-arithmetic.md) defines these boundary behaviors, the result tails, and their use by inference messages.
