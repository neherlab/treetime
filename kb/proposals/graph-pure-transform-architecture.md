...
Every payload access today goes through `Arc<RwLock<>>`. Any code holding a `&Graph` can read or write any node or edge field at any time through `Arc::clone` plus `write_arc()`. This is the Rust equivalent of the reference implementation's monkey-patched node attributes <a id="cite-1a"></a>[Sagulenko et al. 2018](https://doi.org/10.1093/ve/vex042) [[1](#ref-1)]: the type system enforces that a field exists, not that a pass may touch it. The consequences are a locking cost on every access even in single-threaded passes, no compile-time guarantee that a pass reads only its inputs and writes only its outputs, and a payload type such as `struct NodeTimetree` [packages/treetime/src/payload/timetree.rs#L19](../../packages/treetime/src/payload/timetree.rs#L19) that accumulates six inference stages (ancestral, time, clock regression, relaxed clock, outlier detection, and confidence) behind `Option` fields populated at different times. Which fields are valid depends on which passes have run, and nothing in the type records that.
...
The two planes read each other bidirectionally:
...
The four inference passes are one algorithm in four algebras. Each backward sweep gathers a node's children, combines their messages with a commutative operator, applies node-local factors, and transfers the result across the parent edge; each forward sweep removes the recipient's own contribution before transferring back. The combine and the backward edge transfer differ only by algebra:
...
This shared skeleton is the strongest case for the transform model of `D3`: one driver over the topology, parameterized by per-pass operators, would replace the per-command pass closures. The operator set is wider than three functions, though; each pass also supplies:
...
- **Regression**: subtraction, numerically safe [packages/treetime/src/clock/clock_regression.rs#L184](../../packages/treetime/src/clock/clock_regression.rs#L184).
...
Profile first, on the largest datasets (`sc2/4500`, `dengue/2000`), to quantify the lock and `BTreeMap` costs with `dev/bench-graph-pass-cli`, which already sweeps thread counts and diffs outputs. Prototype D1 and D5 in isolation, since a dense keyed store and a pre-computed adjacency index are independently valuable and measurable. Prototype the transform model on clock regression first, not the marginal pass: it is a pure commutative-monoid accumulation with an affine edge transfer and a subtraction-based contribution-removal, carries no normalizer, and lives on a single graph-payload plane, so it exercises the driver and the schedule seam without the cavity-division and normalization hazards the marginal and dating passes add. Convert one marginal pass second, then reassess whether a shared driver or a copied skeleton better fits the operator set.
...
3. <a id="gloss-3"></a> **Sum-product algorithm.** On a tree, the two-pass message-passing algorithm that computes the marginal of a product of factors, inward then outward; Felsenstein pruning is its phylogenetic instance ([Höhna et al. 2014](https://doi.org/10.1093/sysbio/syu039) [[2](#ref-2)]). [↩](#gloss-use-3)
4. <a id="gloss-4"></a> **Cavity distribution.** In belief propagation, a node's accumulated evidence with its own outgoing contribution removed, so a message flowing back into the node is not counted twice. [↩](#gloss-use-4)
