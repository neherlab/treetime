...
Six methods via `--opt-method`: Newton and Brent in $t$, $\sqrt{t}$, $\ln(t)$ spaces. Default `brent-sqrt` (matches v0). Grid search fallback for non-concave regions. See [optimize-newton-raphson-per-edge](../decisions/optimize-newton-raphson-per-edge.md).
...
| Copy v0's signed convergence check  | v0 uses `deltaLH < LHtol` (signed), accepting any likelihood decrease as convergence. This conflates convergence with failure and works in v0 only because dense soft-EM with damping produces near-monotone progress. See [optimize-signed-convergence-check](../v0-errata/optimize-signed-convergence-check.md). v1's three-condition check (converged, oscillating, worsened-with-revert) is more robust. |
...
- Wu, C. F. Jeff. 1983. "On the Convergence Properties of the EM Algorithm." Annals of Statistics 11(1):95-103. https://doi.org/10.1214/aos/1176346060
