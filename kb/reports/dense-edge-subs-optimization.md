...
This is documented in existing known issues ([N-optimize-dense-iteration-slow](../issues/N-optimize-dense-iteration-slow.md)) and the [OpenBLAS profiling report](dense-openblas-profiling.md). It is the primary lever for dense mode performance improvement and would subsume the per-function micro-optimizations in 5.1-5.3.
