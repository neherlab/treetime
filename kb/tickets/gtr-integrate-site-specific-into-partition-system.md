# Integrate site-specific GTR into partition system
...
- Source: [N-gtr-site-specific-partition-integration.md](../issues/N-gtr-site-specific-partition-integration.md) -- delete after full resolution
- The `is_site_specific: bool` field formerly on `GTR` has been removed
- [M-gtr-per-site-rate-variation.md](../issues/M-gtr-per-site-rate-variation.md) -- simpler feature where only $\mu$ varies per site (shared eigendecomposition); `GTRSiteSpecific` provides the full model (per-site $\mu$ AND per-site $\pi$)
- [sequence_evolution.md](../_raw/sequence_evolution.md) -- design document specifying site-specific models
