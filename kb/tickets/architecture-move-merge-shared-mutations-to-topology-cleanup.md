...
- Source: [N-topology-cleanup-move-merge-shared-mutations.md](../issues/N-topology-cleanup-move-merge-shared-mutations.md) -- delete after full resolution
- `docs/reports/command-relationships/README.md` -- describes the ideal `topology_cleanup` module containing `collapse_edge()`, `prune_short_branches()`, and `merge_shared_mutations()`.
- `docs/reports/iterative-tree-refinement/10-status-and-recommendations.md` -- shared-module extraction recommendation (collapse done, merge pending).
